...
- **Two gaps filed rather than worked around silently, and BOTH have since shipped** — `gh-branch:COMMIT_SHA` renders an arbitrary commit's run list (#1083, closed by PR #1101, which also left a comment saying three claims in that issue's own body are wrong), and `gh-labels:tally=PREFIX` tallies label distribution across open issues (#1084) — the cohort burn-down this skill asks me to report every tick, in one call:
  ...
  **That call no longer answers, measured 2026-08-14, and the per-cohort burn-down is currently unobtainable in one call.** The family passed the op's 14-label ceiling at 16, so it refuses; `GH_LABELS_TALLY_MAX=16` gets past the refusal and returns **34 of 34 cells UNKNOWN**, because the tally costs 2 search calls per label against a 30-a-minute limiter. Both remedies the error names are dead — no string prefix selects a useful subset of a numbered family, and raising the ceiling is what produces the empty board. Filed as **#1668**. What still works, and is what to report meanwhile: the same call's `Multi-label:` line states the **total open count**, so the cap check survives even when every per-cohort cell does not.
