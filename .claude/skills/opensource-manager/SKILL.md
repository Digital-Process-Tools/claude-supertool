...
- **Two gaps filed rather than worked around silently, and BOTH have since shipped** — `gh-branch:COMMIT_SHA` renders an arbitrary commit's run list (#1083, closed by PR #1101, which also left a comment saying three claims in that issue's own body are wrong), and `gh-labels:tally=PREFIX` tallies label distribution across open issues (#1084) — the cohort burn-down this skill asks me to report every tick, in one call:
  ...
  **That call answers again as of #1628, and this paragraph said the opposite for one day — correctly, at the time.** It cost 2 search calls per label against a 30-a-minute limiter, so at 15 cohorts it refused and at 16, past `GH_LABELS_TALLY_MAX`, it returned **34 of 34 cells UNKNOWN**. The open column is now a client-side group-by over the one issue listing the op already made — the same listing whose `Multi-label:` line kept working on the run where every cell was `?`, which is what gave the fix away — so a family costs `N + 1` searches and the ceiling is 24. Verified live at 16 cohorts, complete board, no env var.

  Two things that follow for this tick. **The `Open:` and `Closed:` header lines are separate sentences**, because the columns fail independently: a rate-limited minute now costs the closed half and leaves the open counts — the number this rule turns on — intact. And **an open cell can be a floor**: if the listing hits `GH_LABELS_ISSUE_CAP` (400) the open column and `frozen` render `>=N`, so read the `Open:` line before quoting a number as exact.
