...
- **Changelog fragments**: never edit `CHANGELOG.md` directly in a PR — add `changelog.d/<issue>.<section>.md`. Section = `added`/`fixed`/etc. Enforced by `.github/workflows/oss-changelog.yml`. **And nothing outside `changelog.d/` may name a pending fragment by path** — the tag deletes it, which reddened four release commits before `tests/test_changelog_findable_1293.py` started refusing it (#1293).
