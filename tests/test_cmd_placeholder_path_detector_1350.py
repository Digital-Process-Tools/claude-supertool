...
the very same `parts[1]` that `{file}` does. 21 shipped ops carry `{arg}`; 8 of
those name a path in `syntax` and are already held by the syntax detector,
leaving 13 that use it for a handle, a ref, a tag, an ID or a repo slug and take
no path. Promoting it would refuse those 13 and gate nothing. `{file}` and
`{dir}` are the placeholders whose NAME is the claim. The 21/8/13 split, and the
...
class TestTheShippedRegistryIsFullyDetected:
    ...
    def test_the_register_header_comment_counts_are_the_registry_counts(
        ...
        # moved by two, the 19-strong undeclared register still did not.
        # #532 adds `worktree`, the same `"paths": {"args": []}` shape again
        # — PATH there deliberately points outside cwd, at a sibling
        # worktree of the same repo, so a `parts` index would validate the
        # wrong thing. Named and declared each moved by one, the 19-strong
        # undeclared register still did not. The
        # #227 adds `youtube_comment`, the same `"paths": {"args": []}` shape
        # once more: its own `file://PATH` sits inside a pipe-separated field
        # of argument 1, not in an argument slot of its own, so there is no
        # index for a boundary to gate and `safe_resolve_body_path` is the
        # chokepoint that actually contains it. `bluesky_publish` has the
        # identical shape and is in the 19-strong undeclared register, which
        # is what grandfathering looks like from here. Named and declared
        # each moved by one, the register still did not. #2593 adds
        # `youtube_reply`, the identical shape once more (its own
        # `file://PATH` sits inside the same pipe-separated field), so named
        # and declared each moved by one again -- the 19-strong undeclared
        # register still did not. The
        # register's own header comment in `_supertool.py` now reads "33
        # shipped PRESET ops name a path, 14 declare a boundary, these 19 do
        # not" — these four numbers are the only thing under it.
        assert len(named) == 33, sorted(named)
        assert sorted(declared) == [
            "claims", "classify", "gh-issue-comment", "gh-job", "gh-pr-edit",
            "gl-api", "gl-job", "slack_publish", "worktree", "xml",
            "xml_attr", "xml_count", "youtube_comment", "youtube_reply"
        ], sorted(declared)
        ...
        assert len(named_all) == 33, sorted(named_all)
