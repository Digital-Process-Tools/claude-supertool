...
rate-limit-shaped failures from this channel's own watcher rows into
`ratelimit.render_budget_lines`'s DISAGREEMENT line (#2525).
"""
...
def test_reads_state_only_for_gh_sources_rows_and_classifies_it() -> None:
    ...
    def fake_read_state(source, watcher_id):
        return states.get((source, watcher_id), {})

    with mock.patch.object(dispatcher.transport, "read_state", side_effect=fake_read_state) as m:
        out = dispatcher._active_gh_rate_limit_errors(rows)
...
def test_a_clean_gh_source_row_contributes_nothing() -> None:
    ...
    with mock.patch.object(dispatcher.transport, "read_state", return_value={}):
        assert dispatcher._active_gh_rate_limit_errors(rows) == []
