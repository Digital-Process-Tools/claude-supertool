...
def test_the_suggested_rerun_carries_no_quotes(tree: Path) -> None:
    """`grep` does not strip a quote pair -- a quoted pattern is searched
    literally (#1435) -- so a rerun suggestion wrapped in quotes would send
    the caller straight into that other zero, undisclosed (review finding,
    #1777)."""
    out = supertool.dispatch("grep:hello world:c.txt:5")
    assert "grep:(?i)hello world:PATH" in out, (
        "the suggested rerun must be unquoted, or it re-triggers the "
        "#1435 quote-pair zero instead of resolving this one: " + repr(out))
    assert "grep:'(?i)" not in out, repr(out)


def test_the_suggested_rerun_actually_works(tree: Path) -> None:
    """Follow the note's own advice and confirm it lands on the hit."""
    zero = supertool.dispatch("grep:hello world:c.txt:5")
    assert "0 results" in zero, repr(zero)
    rerun = supertool.dispatch("grep:(?i)hello world:c.txt:5")
    assert "1 results" in rerun, (
        "the suggested rerun did not resolve the zero it was suggested "
        "for: " + repr(rerun))


def test_a_letter_free_pattern_gets_no_probe_at_all(tree: Path) -> None:
    """A pattern with no case-sensitive character cannot be case-ambiguous,
    so the probe -- a second full-corpus walk -- must not run at all
    (review finding, #1777: an unconditional probe cost ~3x on the
    common, genuinely-absent zero)."""
