...
def test_init_never_writes_introduction_or_output_format_keys(tmp_path, monkeypatch) -> None:
    """#2342 gave `op_introduction`/`op_output_format` a shipped fallback
    reached when .supertool.json sets neither key. `init` -- the op whose
    job is bootstrapping that file -- must keep emitting neither: the
    fallback already covers a freshly-init'd repo the same as any other
    unconfigured one, and writing it into generated bytes would silently
    duplicate a default that lives in code."""
    repo = _repo(tmp_path)
    monkeypatch.chdir(repo)
    preview = supertool.op_init("")
    preview_data = json.loads(_preview_json(preview))
    assert "introduction" not in preview_data
    assert "output-format" not in preview_data

    supertool.op_init("write")
    data = json.loads((repo / ".supertool.json").read_text(encoding="utf-8"))
    assert "introduction" not in data
    assert "output-format" not in data
