...
def _fake_gh(argv, timeout=20):
    if argv[:2] == ["gh", "api"] and "commits/" in argv[2]:
        # 25 minutes ago, computed off the real clock (`branch._age_secs`
        # reads `datetime.now`, not `time.time`, so the `time.time` patch
        # below does not move it): past `_GRACE` (15min) so this is not
        # "still expected", but well under the #2362 `NO_RUN_STALE_SECS`
        # (45min) threshold this file predates -- a fixed past date used to
        # mean "long past grace" forever, until that second threshold
        # existed.
        stale_free_date = (datetime.now(timezone.utc) - timedelta(seconds=1500)
...
def test_snapshot_reaches_no_run_through_the_real_gh_boundary() -> None:
    ...
    with mock.patch.object(branch, "_gh", side_effect=_fake_gh), \
        ...
        (state, sentence, sha, repo, error, has_failed_leg,
         has_unread_jobs) = poller._snapshot("main")
    assert error == "", error
    assert state == branch.NO_RUN, (state, sentence)
    assert sha == "c391c1333b6793f4fc2e5a2cc830024fd834ffe1"
    assert repo == "OWNER/REPO", repo
    assert has_failed_leg is False, "NO_RUN has no legs to have failed"
    assert has_unread_jobs is False, "NO_RUN has no job list to have missed"
...
def test_snapshot_reports_the_lookup_failure_when_the_commit_cannot_resolve() -> None:
    ...
    assert has_failed_leg is False, "an unresolved commit has no leg data to have an opinion about"
    assert has_unread_jobs is False, "an unresolved commit has no job list to have missed"
