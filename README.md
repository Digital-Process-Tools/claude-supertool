...
- **`gh-job` / `gh-run` / `gh-branch`** — a job's failure detail from either GitHub id namespace, a run's job table under a header that sums it, and *is this branch green* answered for the ref that has no PR after a squash merge. [Details](docs/presets/github.md#two-id-namespaces-actions-jobs-and-check-runs). What `gh-job:ID:fail` actually surfaces about a red leg, measured against six real failed runs on this repository's own history: [docs/ci-failure-classification.md](docs/ci-failure-classification.md).
...
A caller that is read-only by design -- a review or audit agent whose whole remit is annotating a diff, never acting on it -- can declare so and be held to it: `SUPERTOOL_READ_ONLY=1` refuses any op that `ops:roster` does not class `read-only`, naming the op and its class rather than a bare denial. It constrains only a cooperative caller, not an adversary -- the same op layer a determined writer can simply not call -- and is documented as exactly that: [docs/configuration.md#caller-declared-read-only-mode](docs/configuration.md#caller-declared-read-only-mode) ([#1787](https://github.com/Digital-Process-Tools/claude-supertool/issues/1787)).
...
- **Two files, one of them a shim, plus a third that only the pip route reaches.** `supertool.py` is the entry point everything invokes and is 171 lines; the tool itself is `_supertool.py` beside it. The split exists so CPython caches the bytecode: a script named on the command line is recompiled from source on every run, an imported module is not, and that recompile measured ~145ms per invocation on ubuntu and windows runners ([#931](https://github.com/Digital-Process-Tools/claude-supertool/issues/931)). Still no package layout, no required deps — clone or `pip install`, both work, and since [#1783](https://github.com/Digital-Process-Tools/claude-supertool/issues/1783) `pip install` also ships `_shipped_reference.py`, a generated module carrying the `builtin-ops` half of `.supertool.json` so `help:OP` still answers for a project that documents nothing of its own — the clone and plugin routes read `.supertool.json` itself and never touch it.
