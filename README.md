...
| `vi` | `vi:::PATH:::SCRIPT` | vi-flavored cursor-based multi-action edit. SCRIPT actions (separated by `␞` U+241E or real newline) take the form `[count][verb][arg]`. **Cursor:** `gg`/`G` (BOF/EOF), `nG` (goto line), `0`/`$` (BOL/EOL), `/PAT`/`?PAT` (search fwd/bwd), `nh`/`nl`/`nj`/`nk` (move). **Inserts** (TEXT runs to end of action, `\n`/`\t` decoded): `iTEXT` (before), `aTEXT` (after), `ITEXT` (BOL), `ATEXT` (EOL), `oTEXT` (open below), `OTEXT` (open above). **Deletes:** `x`/`nx`, `dd`/`ndd`, `D`. **Replace:** `rc`. **Self-contained** — `/PAT` removes the need for a prior Read. Token-cheap for many small edits. `␞` was chosen because it never appears in code or text, so TEXT can contain `;`/`{`/`}`/etc. with no escaping. Example: `vi:::skill.md:::/## Process␞O## Task list␞o1. Foo␞o2. Bar`. |
