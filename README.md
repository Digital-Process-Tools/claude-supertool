...
- **`gh-labels`** — *what can I tag this with?* The repo's label vocabulary, grouped by name prefix, with how many **open** issues carry each one. The first call of any triage run, and the spelling is not portable between repos — `priority-high` here, `priority:high` in `claude-remember`. A count is exact, a `>=N` floor when the issue read hit its cap, or `?` when the issue list could not be read — never `0` for "I did not look".
