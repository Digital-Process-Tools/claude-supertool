...
- **`gh-prs`** — PR triage board, failing-first. It also **says whose board it is**: `gh-prs` means "my PRs" (`author=@me`), and that default used to be invisible *and* unreachable — on a repo whose only open PRs came from outside contributors it printed `No PRs match.` / `0 PR(s)`, the strongest available statement of absence, about the rows a maintainer board exists to surface. The default stays; the render now separates rows found, nothing-because-the-filter-excluded-`N` (with the count, and `anyauthor` to see them), nothing-open, and a stated `UNKNOWN` when the check itself could not run. `anyauthor` drops the default; asking for it alongside an explicit `author=` is refused rather than resolved by precedence ([details](docs/presets/github.md#gh-prs-says-whose-board-it-is)).
