...
| `vi` | `vi:::PATH:::SCRIPT` | vi-flavored cursor-based multi-action edit. SCRIPT is parsed like a real vim macro: chars are verbs in NORMAL mode; insert verbs (`i`/`a`/`A`/`I`/`o`/`O`), search (`/`/`?`), and ex (`:...`) enter "greedy" modes where all following chars are TEXT/PAT until `\e` (ESC, U+001B) returns to NORMAL. **No separator chars** — `;`, `{`, `}`, newlines etc. are literal data, never special. **Cursor:** `gg`/`G` (BOF/EOF), `nG` (goto line), `0`/`$` (BOL/EOL), `/PAT`/`?PAT` (search fwd/bwd), `nh`/`nl`/`nj`/`nk` (move). **Inserts** (TEXT runs until `\e` or EOS, `\n`/`\t` decoded): `iTEXT` (before), `aTEXT` (after), `ITEXT` (BOL), `ATEXT` (EOL), `oTEXT` (open below), `OTEXT` (open above). **Deletes:** `x`/`nx`, `dd`/`ndd`, `D`. **Replace:** `rc`. **Self-contained** — `/PAT` removes the need for a prior Read. Example: `vi:::skill.md:::/## Process\eO## Task list\eo1. Foo\eo2. Bar`. |
