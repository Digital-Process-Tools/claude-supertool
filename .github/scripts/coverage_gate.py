...
#: the reasons are not the same one. Lumping three directories under a single
...
NOT_MEASURED_PY: "dict[str, str]" = {
    "_shipped_reference.py": (
        "generated by .github/scripts/generate_shipped_reference.py (#1783) "
        "from this repo's own .supertool.json -- its body is one JSON string "
        "literal assigned to BUILTIN_OPS via json.loads(), so 'coverage' of it "
        "would only measure whether that one line executed, which importing it "
        "at all already guarantees. Correctness is a staleness comparison "
        "against the generator's own render(), not a line-execution count; "
