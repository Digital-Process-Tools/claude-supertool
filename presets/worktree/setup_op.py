...
def _do_exclude(target: Path, exclude_entries: tuple, lines: list) -> None:
...
def run(target: Path) -> "tuple[int, str]":
    ...
    if link_entries:
        ...
        for entry in link_entries:
            _do_link(entry, primary / entry, target / entry, lines, manifest)

    if copy_entries:
        lines.append("copy:")
        for entry in copy_entries:
            _do_copy(entry, primary / entry, target / entry, lines, manifest)

    if exclude_entries:
        lines.append("exclude:")
        _do_exclude(target, exclude_entries, lines)

    try:
        _common.write_manifest(target, manifest)
    except _common.TargetError as exc:
        lines.append(f"  WARNING could not save the provisioning manifest, teardown may miss entries: {exc}")
