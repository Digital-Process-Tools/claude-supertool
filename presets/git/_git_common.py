...
# ── the git invocation chokepoint ────────────────────────────────────────
#
# Every name below used to be DEFINED here, and moved to `presets/_git_run.py`
# in #2447. Nothing about what any of them does changed; what changed is that
# they now sit at the `presets/` root, where `presets/dashboard/dashboard.py`
# and `presets/github/pr_merge.py` can import them without either of those
# reaching into this preset's own directory. Both carried a private `_git()`
# on a bare `subprocess` call until then, with no lock retry, no `_stop()`
# grace and no `SUPERTOOL_GIT_TIMEOUT`, and #1945's `--no-optional-locks` had
# to be swept into all three by hand.
#
# Re-exported rather than re-implemented, and asserted by identity in
# `tests/test_git_invocation_chokepoint_2447.py`: eight presets here import
# these names from this module, and a second copy of `_git` would be the exact
# defect the move exists to close, one directory further along.
from _git_run import (  # noqa: E402,F401  (re-export: this module's own public surface)
    LOCK_WAIT_DEFAULT,
    TIMEOUT_RC,
