...
if __name__ == "__main__":
    # Rejoined on ':' — see the note in presets/hashnode/list.py (#873).
    # `{args}` hands each ':'-token over separately; `parse_args` splits on
    # ':' for the documented `[:N]`, which under `{arg}` never reached it.
