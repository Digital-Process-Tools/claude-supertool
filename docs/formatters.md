...
## `first_changed_line` / `last_changed_line` — where an unrelated rewrite landed (#2405)

`ruff-format` also names the 1-indexed span of *before-file* line numbers its rewrite actually touched, as `metrics.first_changed_line` / `metrics.last_changed_line` (`null`/absent when the run changed nothing — never a fabricated `(1, 1)`). The counts alone (`lines_added`/`lines_removed`) say *how much* moved; this says *where*, in line numbers a same-session read taken moments earlier would still recognise.

The gap this closes: a formatter runs automatically after every mutating op, and it is free to reformat lines the triggering edit never touched — collapsing a multi-line condition onto one line, say. A read taken just before that write, of a *different* region of the same file, is not guaranteed to still match byte-for-byte once the formatter has run. `_formatter_render_row` now appends `(lines N-M touched -- an earlier read of that region is now stale, re-read before reusing it)` to the row whenever the range is present, in the same call whose output already carries `+N -M` — so the caller learns this without a second round-trip, at the moment it matters, before composing an `edit` whose `old` string might now be stale.

Only the four SCHEMA-adapter formatters can report this (they alone re-read the file to compute a diff at all); a raw-`cmd` formatter's row is unaffected. All four now compute the range, sharing one implementation — `validators/common/line_diff.py`'s `line_diff()` — rather than `ruff-format` carrying its own copy and the other three drifting from it (#2458). `php-cs-fixer`, `prettier-write` and `phpcbf` used to report counts only; each now imports `line_diff` the same way `ruff-format` does and reports the identical `first_changed_line` / `last_changed_line` shape, including both edge cases below.

Two edge cases the self-review pass on #2405 found and pinned with tests: a pure insertion into a file that was completely empty before the write reports `(None, None)`, never a fabricated `(1, 1)` — there is no before-line 1 to have gone stale, because none existed. And two small, far-apart hunks (e.g. before-lines 2 and 10 of a 10-line file) are reported as **one merged span** covering everything between them, not as two separate ranges — an intentional over-approximation (it never under-reports; re-reading the whole span will not miss a touched line), not a bug.
