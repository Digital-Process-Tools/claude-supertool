...
Only the four SCHEMA-adapter formatters can report this (they alone re-read the file to compute a diff at all); a raw-`cmd` formatter's row is unaffected, and `ruff-format` is the only one of the four that computes the range so far — `php-cs-fixer`, `prettier-write` and `phpcbf` still report counts only.
