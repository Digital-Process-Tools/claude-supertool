...
|----|--------|--------------|
| `grep` | `grep:PATTERN:PATH` or `grep:PATTERN:PATH:LIMIT` | 10 results default, code + doc extensions only. Each output line is capped at 500 chars (`grep.max_line_chars`) with a `… (+N chars)` marker — see [per-line cap](#per-line-cap); append `:full` ([#1712](https://github.com/Digital-Process-Tools/claude-supertool/issues/1712)) to suppress that cap for this call only. **Auto-reads** full file if PATH is a concrete file under **both** the byte cap (< 20KB) **and** the line cap (≤ 60 lines, `MAX_AUTOREAD_LINES`) with a match; over the line cap it prints a `[auto-read skipped: > N lines — read:PATH:full to see it]` note instead of dumping the file. Append `:no-auto-read` to suppress entirely (parity with `glob`). |
...
- **It does not guess, and it says which way it declined.** rtk absent, failing or timing out; `-rc` output that is not `path:N`; a non-ASCII decimal `int()` would silently have converted; or the switch below turned off — no total came back at all, and the report reads `— TRUNCATED, more matches exist, total unknown (the delegated count pass returned no total)`. A census that *did* answer and was **refused** — because its total was at or below the number of rows printed under it, the two passes being separate reads of a tree that can change between them — says so instead: `total unknown (the delegated count came back at 2, at or below the 3 rows shown, and was refused as incoherent)`. One sentence for both would be a receipt misreporting its own mechanism, which is the defect this whole marker exists to close. Either way no number is printed, and neither is the same sentence as a count that came back small.
