...
**Excluded files are filtered on the way back, not just on the way out.** Single-segment entries reach grep as both `--exclude-dir=NAME` and `--exclude=NAME` — `--exclude-dir` alone cannot skip a *file*, which is how a delegated `grep` printed the contents of every `.env` in the tree ([#691](https://github.com/Digital-Process-Tools/claude-supertool/issues/691)). But the argv is the optimisation, not the guarantee: whatever comes back is re-filtered through the same matcher the native walker uses, so a grep that ignores `--exclude` still cannot leak. System grep has no way to express a negation (`!.env.example`), so when the effective list carries one, wildcard entries are withheld from the argv and left entirely to that filter. If the filter drops anything, the search is redone natively — the delegated header's count, its `limit + 1` truncation probe and its `?` denominator all describe the unfiltered result set, and a header describing a result set that no longer exists is exactly the kind of quiet lie the `?` was introduced to avoid.
