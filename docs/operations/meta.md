...
| `ops` | `ops` | Every op's **signature** — built-in ops, custom ops and aliases, one row each, no descriptions and no examples (~4.71KB here). Its footer says what the **whole `ops:full` render** costs, not what the descriptions cost: that is the footer's number minus this listing, 85,009 bytes here (#2675 widened `channel:health`'s description to name its sixth state, `BOUND, UNPROVEN` (exit 8) -- on top of #2665's `gh-issue-comment` description for its GET-before-PATCH ownership check on `edit=COMMENT_ID` -- on top of #2643's `gh-issue-comment` description for the `edit=COMMENT_ID` token itself -- on top of #2592's git-commit control-byte refusal to its own description -- on top of #1985's `oss-tick` op's description -- on top of #1315's `gl-pipeline`'s `:full` token in its `syntax` and `mcp_stop`'s `--all` pointer to `mcp_stop_all` in its `description` -- on top of #2536/#2544's `SUPERTOOL_WATCH_DESKTOP`/`SUPERTOOL_WATCH_NO_DESKTOP` knobs on the `watch` op's description -- `radar`'s own description was left alone, since it was already in the #1774 over-budget ledger and cannot grow -- on top of #1850's `statusline` op, #2472's `gh-mirror`/`gh-pr` descriptions, #2429's `worktree` `:force` flag, #1955's `gh-mirror` and #1822's `json-set`, moving this figure again). It also names the two ops that fetch them (#1774). |
| `ops:full` | `ops:full` | The same rows carrying their descriptions and examples — what `ops` was before #1774, ~89.76KB here (moved from ~89.55KB by #2665's `gh-issue-comment` GET-before-PATCH ownership-check description, itself moved from ~89.11KB by #2643's `gh-issue-comment` `edit=COMMENT_ID` description, itself moved from ~88.71KB by #2592's git-commit control-byte refusal, itself moved from ~87.46KB by #1985's `oss-tick` op description, itself moved from ~87.11KB by #1315's `gl-pipeline`/`mcp_stop` description and syntax text, and from ~86.89KB by #2536/#2544's desktop-notification knob text). This is the full reference; nothing was deleted when the default changed. || `ops-compact` | `ops-compact` | The descriptive listing with per-op detail trimmed except where an entry declares `hint` (~19.77KB). Still over the 10,000-byte SessionStart hook cap; it says so in its first line rather than letting the tail be cut silently. |
| `ops:roster` | `ops:roster` | Every op name plus a safety class and nothing else (~2.1KB, moved from ~2.0KB by #1850's `statusline` op) — the only listing that fits the SessionStart cap, so it is what the hook prints. Unmarked = read-only, `*` = writes in this tree, `!` = acts outside it or outlives the call. |
| `ops:grep=` | `ops:grep=PATTERN` | Rows whose name, syntax or description match `PATTERN` — a search, not a fourth mode. Always states `N of M ops matched`, `0 of M` included, so an empty result cannot be mistaken for a short roster (#1318). See below. |
...
| `init` | `init` or `init:write` | Derive and write a starter `.supertool.json` for the current repo when none exists yet -- previews by default, `init:write` commits. See below. |
...
## `init` — bootstrap a starter `.supertool.json`

Without a `.supertool.json`, every preset op (`gh-pr`, `gh-issue`, `git-trail`, `gl-mr`, ...) simply does not exist in that repo ([#614](https://github.com/Digital-Process-Tools/claude-supertool/issues/614)) -- there is no fallback, only silence. `init` derives what it safely can from the repo itself, and refuses -- writing nothing -- everywhere it cannot ([#858](https://github.com/Digital-Process-Tools/claude-supertool/issues/858)):

- **`defaults.github_repo` / `defaults.gitlab_project`** from the `origin` remote. No remote, an unparseable URL, or a host that is neither `github.com` nor a recognised GitLab host: declines rather than emit a plausible-but-wrong repo slug.
- **`presets`**: the matching platform preset, `git` always, `xml` only if a tracked file has that extension. `mcp`/`watch` stay opt-in -- both need machine-specific setup `init` cannot infer.
- **`validators`**: `jsonlint` always (its adapter is stdlib `json.load`, nothing to resolve). `ruff`/`shellcheck` only when both a matching tracked file exists **and** the adapter's own binary actually resolves on `PATH` -- declaring a validator that cannot run is the silent-pass class this repo's own contract forbids.
- **`rtk` / `allow_outside_cwd` / `allow_vim_shell`**: always `false` -- safe defaults, not copies of whatever repo last ran `init`. This repo's own `.supertool.json` sets `allow_outside_cwd: true` because it is supertool's own checkout; a stranger's repo must not inherit that.

Two more refusals, about the write itself rather than what to write: an existing `.supertool.json` is never overwritten, hand-written or not -- `allow_vim_shell`/`allow_outside_cwd` both widen what supertool may do, so a silent rewrite is worse than doing nothing. Merging new presets into an already-edited config ("auto change it for you" for a *hand-written* file) is a harder, separate problem the issue explicitly leaves to a follow-up; `init` only ever creates a file that does not exist yet.

```bash
./supertool 'init'         # preview -- prints the JSON, writes nothing
./supertool 'init:write'   # commits it
```
...
`ops:full` and `ops:roster` are modes rather than filters — a fixed set of three words, each rendering every op, and a fourth unrecognised word is refused naming them (#1774).

**`ops:grep=PATTERN` is a filter, and a deliberately different one (#1318).** The three modes above render every op; this searches op name, syntax and description for `PATTERN` and prints only the matching rows — the route three independent agents reached for and could not reach, because piping `ops` through `grep` is what the raw-command guard correctly blocks, leaving a redirect-to-temp-file workaround as the only way around it. What made a filter refused in the first place still applies and is still answered: `ops:gh-labl` matching nothing must never render identically to an op that does not exist, so the filter always states `N of M ops matched` — `0 of M` included — rather than a bare empty listing (`M` is the true dispatchable total on this checkout, not a figure to pin in prose). `ops:grep=` with nothing after it is refused rather than treated as matching everything. Case-sensitive, like every other pattern slot in this file, and searches every dispatchable op — including one with no `.supertool.json` entry — not only the ones a config section describes.
