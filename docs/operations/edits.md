...
It applies to `edit` (colon CLI and payload route) and, since #938, to `replace` too — each occurrence it finds already sitting inside a prior application counts toward the same `K re-applied` footer, and the file's own receipt line gains a `[N re-applied]` tag next to its occurrence count. `vim` is scripted rather than anchored and remains uninstrumented — its substitutions are regex-driven, and "the old text is contained in the new" has no settled meaning once `old` is a pattern rather than literal text (#938 tracks this as the open residual).

Interdependent edits are the normal case for a batch, and a half-applied set is rarely wanted — but `batch:` is **not** atomic by default, and this change does not make it so. Set `continue_on_error: false` in the payload wrapper to stop at the first failure; #680 already makes that failure visible (the declined op names itself in the footer, and the call exits non-zero), so a caller has both the signal and the switch today.

**This is deliberately not scoped to grow into real atomicity ([#700](https://github.com/Digital-Process-Tools/claude-supertool/issues/700)).** A batch entry is not necessarily a file write at all: `git-commit`, any `gh-*` op, or another network-side-effecting preset can sit in the same list as `edit`/`paste`, and dispatch does not distinguish them. A git commit or a posted GitHub comment cannot be snapshotted and restored the way a file's bytes can — so a "rollback" that only ever covers the file-writing entries would report the whole batch as undone while an earlier, irreversible entry had already landed. That is a quieter, wronger answer than the partial-application state `batch:` already reports honestly, which is why #700 was closed with this paragraph rather than with rollback machinery: `continue_on_error: false` plus a non-zero exit and a named failure is the ceiling for a tool whose entries can be arbitrary side effects, not just edits.

Preview ops (`replace_dry`) and read-only ops get no `[result]` — a read op's own count line is already the last thing printed, so nothing intervenes to misread.
...
```text
ERROR: @file payload refused (p.toml): a ''' literal block carries an EVEN run of backslashes (\\, \\\\, ...) in a field that is WRITTEN to the file, and a literal block processes NO escapes -- the run would reach disk at its full length, pass every validator, and be wrong only in string contents. Each occurrence below names the run it found -- if you already know this run is meant AS WRITTEN (documentation of the sequence itself, not a doubling mistake), skip straight to `literal_backslashes` at the end of this message. (#2314)
  ↳ `new` -- 1 even backslash run, all shown:
      1/1 at payload line 3, column 28 -- a run of 2:
          PAT = re.compile("\\d+")
                            ^^
  ↳ the target here is .py -- a doubled backslash there is often a REAL one (a JSON string escape, a Python string-literal escape) rather than an escape reflex: the SECOND fix below is the common case for this extension, not the first.
...
**The refusal's own pointer to the override moved, and a second hint was added, without either changing when it fires** ([#2314](https://github.com/Digital-Process-Tools/claude-supertool/issues/2314)). The mention of `literal_backslashes` used to sit only at the end of the message, after every occurrence was listed; it now opens the message too, so a caller who has already met this refusal and knows they want the override does not have to scroll past a located-occurrence block to find it. A second hint, keyed off the payload's own `path` extension ([#1794](https://github.com/Digital-Process-Tools/claude-supertool/issues/1794)), follows the occurrence list and points at whichever fix is the common case for that extension: `.py`, `.json`, `.js`, `.jsx`, `.ts`, `.tsx`, `.jsonc`, `.yaml`, `.yml` targets get pointed at `literal_backslashes`, because a doubled backslash reaching one of those files is usually real — a JSON string escape, or a Python string-literal escape that the *target file's own parser* will consume, not TOML's — while `.md`, `.txt`, `.rst`, `.adoc` targets get pointed the other way, because prose rarely wants the pair as written. Mixed extensions, or one this repo has no opinion about, get no hint at all rather than a guess.

**The odd-run exemption below is what a fixture-writing test hits, in practice.** Writing a Python string literal that itself contains a single `\n` — `'{"type": "x"}\n' * 51`, say, building JSONL fixture content one record at a time — is exactly the odd-run case: it writes through with no refusal at all. What trips the guard instead is the *doubled* form of the same idea: two literal characters, `\` and `n`, that some parser other than TOML's needs to see — a JSON string's own escaped newline reaching disk inside a Python source string, spelled `\\n` in the payload.
