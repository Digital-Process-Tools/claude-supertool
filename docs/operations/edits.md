...
The op is built in and always available. Its **documentation** ships in the [`vim` preset](../presets/vim.md) ([#2026](https://github.com/Digital-Process-Tools/claude-supertool/issues/2026)), so `help:vim` and the grammar in `ops-compact` need `"presets": ["vim"]` in the project's `.supertool.json`. The table below is the same grammar and does not.
