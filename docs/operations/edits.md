...
**The guard only ever fires on an EVEN run**, which is worth stating plainly because it is easy to expect the opposite. Writing a Python string literal that itself contains a single `\n` — `'{"type": "x"}\n' * 51`, say, building JSONL fixture content one record at a time — is an ODD run of one backslash per occurrence and writes through with no refusal at all: doubling cannot produce one, so this shape never reaches the guard in the first place. What the guard is for is the doubled case: two literal characters, `\` and `n`, that some downstream parser other than TOML's needs to see — a JSON string's own escaped newline inside a Python source string, for instance, spelled `\\n` in the payload.
