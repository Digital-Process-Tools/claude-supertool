...
**Affected.** The macro grammar: the NORMAL/GREEDY mode boundaries, the cursor verbs, the ex subset, the autocorrections. That is 1,373 bytes carrying `hint: true`, which is why it renders in `ops-compact` — the listing that runs against the SessionStart byte cap — rather than only in `ops:full`. Measured on this repository, leaving the preset unlisted saves 1,478 bytes in `ops-compact` and `ops:full`, and 19 in the default `ops` listing, which has been signatures-only since [#1774](https://github.com/Digital-Process-Tools/claude-supertool/issues/1774).

## Whether to list it

List it if anything in the repo edits with `vim`. The saving is real and so is the cost: an agent that reaches for the op the description itself calls the **default edit op**, and cannot read the grammar, writes a macro that does something other than what it meant. `help:vim` goes dark with the listing.

Leave it unlisted in a repo that edits with `edit`, `paste` and `replace_lines` and never reaches for a macro. `vim` still works there; it is simply undocumented, and `ops` says so under its "Also accepted, no reference in .supertool.json" line rather than staying silent about it.
