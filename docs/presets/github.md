...
**`radar`'s GitHub tier still defaults to `author=@me`.** It calls `_build_list_cmd` with two positional arguments, and that parameter default is deliberately unchanged: radar keys its departure snapshot on the filter *string*, which an unfiltered board spells the same way as today's, so widening it here would replay every previously-hidden PR as a fresh arrival on every watcher, once, and leave the tier's own `scope author=@me (default)` label false. That half of #1207 belongs in `presets/watch/`, with its snapshot key.
