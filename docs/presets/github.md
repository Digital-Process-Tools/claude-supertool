...
| `gh-since-tag` | **retired in [#1405](https://github.com/Digital-Process-Tools/claude-supertool/issues/1405)** — it is `gh-prs:merged-since=TAG,state=merged` now, and the entry survives as a tombstone that prints the mapping rather than becoming an unknown token |
...
|---|---|
| Does it take a **tag name**? | **No — instants only.** Resolving a tag has three outcomes (`RESOLVED` / `AMBIGUOUS` / `UNRESOLVED`) and a listing filter has two: apply the token, or refuse it. There is no way here to say *two tags at one instant on different commits*. And `gh-prs` follows a `repo:OWNER/NAME` target while a tag is a read of the **local** clone, so a tag name would resolve against one repository and list from another — measured, that combination printed one repo's tag beside another repo's merge count. |
| What spellings parse? | An ISO date or instant, through the same parser `gh-since-tag` places its rows with. `2026-08-09`, `2026-08-09T16:07:45Z` and `...+02:00` all normalise to one canonical UTC qualifier. A bare date is that day at `00:00:00` UTC. Anything else is **refused**, not dropped — a value the argv builder skips leaves the *unbounded* board answering a bounded question ([#939](https://github.com/Digital-Process-Tools/claude-supertool/issues/939)). |
| What can you actually type? | **The date form, and only that.** supertool splits an op argument on `:`, so `merged-since=2026-08-09T16:07:45Z` arrives as three argv entries and the value is gone before any filter is parsed. The refusal for that names `merged-since=YYYY-MM-DD` rather than closing with the generic *query the backend CLI directly and file the gap*, which is right when nothing can express the value and wrong here. In-process callers pass the full instant. |
...
Since [#1405](https://github.com/Digital-Process-Tools/claude-supertool/issues/1405) this is a **filter on `gh-prs`** — `gh-prs:merged-since=TAG,state=merged` — and `gh-since-tag` is a tombstone that prints the mapping. See [The release gate is a filter now](#the-release-gate-is-a-filter-now). Everything below is unchanged apart from where it is spelled, with the one exception marked in the boundary table.
