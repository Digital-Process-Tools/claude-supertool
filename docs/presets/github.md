...
| `gh-mirror` | `gh-mirror:(issue\|pr):NUMBER` | What the write-through mirror ([#1955](https://github.com/Digital-Process-Tools/claude-supertool/issues/1955), extended to PRs by [#2472](https://github.com/Digital-Process-Tools/claude-supertool/issues/2472)) currently holds for one issue or PR, with **no network call**. See [The write-through mirror](#the-write-through-mirror) below. |
...
| `gh-issue-comment` | `gh-issue-comment:NUMBER:@FILE[:edit=COMMENT_ID]` \\| `gh-issue-comment:NUMBER:@-[:edit=COMMENT_ID]` | **Comment on an issue, and prove what landed.** The one write in this family with no op: a follow-up scoping note on an issue already filed fell back to raw `gh issue comment --body-file`, which nothing validates and nothing reads back ([#2078](https://github.com/Digital-Process-Tools/claude-supertool/issues/2078)). Takes the same payload shape as `gh-issue-create` (`repo`, `body`/`body_file`), because a comment body is free-form prose with newlines and colons that cannot survive `:`-tokenization -- same key names, and now the same refusal of a non-string `body` too: like `gh-issue-create` ([#2315](https://github.com/Digital-Process-Tools/claude-supertool/issues/2315)), this op refuses a TOML `[[body]]` table-array rather than silently stringifying it into a Python-repr comment body ([#2322](https://github.com/Digital-Process-Tools/claude-supertool/issues/2322)). Writes through REST (`POST repos/{repo}/issues/{number}/comments`), a plain create with no GraphQL field set to go stale under it the way `gh pr edit`'s did. After the write it compares the POST response's stored body against what was sent, byte for byte, in the same four states `gh-pr-edit` uses: `EXACT`, line endings `NORMALISED` by the server, `MISMATCH` naming the first differing line, or `UNKNOWN` when the response carried no body field -- only the first two exit 0. No closing-reference gate: a comment does not replace a published body, so there is nothing an update could drop. A trailing `:edit=COMMENT_ID` corrects a comment already posted instead of creating a new one ([#2643](https://github.com/Digital-Process-Tools/claude-supertool/issues/2643)): a GET of the comment runs first, refusing before any PATCH if its `issue_url` does not match `NUMBER` -- a mistyped or stale `COMMENT_ID` no longer silently overwrites a comment on some other issue or PR ([#2665](https://github.com/Digital-Process-Tools/claude-supertool/issues/2665)) -- then `PATCH repos/{repo}/issues/comments/{id}` in place of the POST, same payload, same four-state receipt, and the disclosure marker re-applied idempotently. The id is already in this op's own prior `[result]` line (`comment id=...`) |
...
Every `gh-*` read op used to render text and throw it away — a maintainer tick asking the same question twice paid a fresh API call for an answer it already had ([#1955](https://github.com/Digital-Process-Tools/claude-supertool/issues/1955)). A scheduled poller was the wrong fix: a cached answer that was correct when taken and false a minute later is indistinguishable from a fresh one at the render, so a poller invents a freshness it cannot have. A **write-through** mirror has no such problem by construction — it is only ever populated as a side effect of a read the caller was already making, so a mirrored file's age *is* the age of the read that produced it.

**Opt-in.** Set a top-level `"gh_mirror_dir"` key in `.supertool.json` (a path, resolved relative to the directory holding that config file, not to the caller's cwd):

```json
{ "gh_mirror_dir": ".max/gh-mirror" }
```

Unset (the default) means the mirror is off: `gh-issue`/`gh-pr` render exactly as they always have, with no extra output and no extra write.

**Scope, as of #2472: `gh-issue` and `gh-pr`.** Each fetches one object's full dashboard reply already, so writing it through is a same-shape side effect for both — issues and PRs land in separate directories (`issues/` and `prs/`, each with its own manifest), so a number that happens to match both never answers for the wrong one. `gh-issues` and `gh-prs` — the **list** ops — are deliberately not wired: their `--json` field sets are a projection (`gh-issues`'s omits `body` entirely) rather than the full dashboard shape, so writing those rows through would either silently downgrade a previously-full mirror entry to a partial one, or need a field-level merge scheme this lane did not size — and a list read returning dozens of rows per call would mean that many manifest-lock acquisitions on exactly the op class (`gh-prs`) this feature's own design constraint says must stay fast and live for collision-avoidance. Because only the single-object ops write through it, **`gh-mirror`'s `not-cached` means "never read via the matching single-object op", not "does not exist on the tracker."** A manifest built from the full list a `gh-issues`/`gh-prs` call already fetches is exactly the piece a list-aware extension would add.

**What every `gh-issue`/`gh-pr` read stores, as a side effect:** the full, untruncated API reply (`gh issue view --json ...` / `gh pr view --json ...`), plus the timestamp of that read — never the display-capped body a non-`:full` render shows. A mirror of a truncated body would lie by omission.

**`gh-mirror:issue:NUMBER`** and **`gh-mirror:pr:NUMBER`** answer what the mirror holds, with no network call, in four renders:
...
| `#N: cached (AGE)` | The matching op (`gh-issue`/`gh-pr`) fetched this number and the mirror is intact. `AGE` is `Nm`/`Nh`/`Nd`. |
| `#N: not-cached` | The mirror is on and intact, but this number was never read via the matching op (see the scope note above). |
...
The fourth state never renders as either of the middle two. `Path.is_file()` silently swallows `PermissionError` and returns `False`, which reads identically to "never fetched" — this repo's own "absence produced by the tool" defect class — so the mirror reader reads the manifest and body directly (`read_text`) rather than stat-checking them first, and every `OSError` distinct from "the file genuinely does not exist" renders `mirror-unreadable`.

**A mirror write failure never fails the `gh-issue` read it rides on.** `_mirror.write_issue` never raises; a failure prints one `note:` line and the primary render proceeds unchanged.

**Never an authority for anything a decision turns on** — the issue's own design constraint. Open/closed state, assignee, labels and linked-PR state stay live reads: a maintainer loop's own collision-avoidance (two agents claiming the same issue) depends on the assignee field being live, never mirrored. The mirror stores exactly what `gh-issue` rendered; nothing here re-derives or infers a current value from it.
...
## Zero runs stops being the same finding at 20 minutes and at 20 hours

A squash merge into `master` produced a commit for which **zero workflow runs
of any status ever existed** — not pending, not cancelled — confirmed ~20
minutes after the merge, with path filters, the documented
cancel-in-progress trade-off and rate limiting all ruled out
([#2362](https://github.com/Digital-Process-Tools/claude-supertool/issues/2362)).
Not reproduced a second time; the observation is a single one.

`no_run_verdict` already had one threshold — `_GRACE`, `~15min`, measured in
[#585](https://github.com/Digital-Process-Tools/claude-supertool/issues/585)
— past which the cause is declined rather than guessed: a path filter and a
workflow that never fired look identical from the run listing alone. But
"declined" meant declined *forever*: the sentence at 20 minutes and the
sentence at 20 hours were the same words, so a commit that will genuinely
never see a run was never distinguished from one that might still be a
minute away.

`NO_RUN_STALE_SECS` (~45min, three times `_GRACE`) is the second, longer
threshold. Past it the ordinary explanations have had long enough to have
already resolved themselves were they going to, and the state escalates to
`NO RUN — STALE` — a distinct token, never a reworded sentence under the
same `NO RUN`, so a caller comparing states rather than scanning prose still
sees the difference:

```
Branch master: NO RUN — STALE
Verdict: NO RUN — STALE — zero workflow runs on 339d04d, head commit 47m old and past
  the ~45min window past which a commit with still nothing recorded is no longer read
  as ordinary listing lag or a plausible path filter — those explanations wear out
  with time and this one already has (#2362). Something is wrong with how this commit
  was expected to trigger CI, or a webhook delivery was dropped outright.
```

The clock is the commit's own age (`age_secs`), the same clock `_GRACE`
already reads — never the moment a particular caller happened to poll —
so a hand-run `gh-branch:<ref>` and `gh-pr-merge`'s post-merge report agree
about which reading applies without either one re-deriving it. `gh-pr-merge`
needed no change to surface this: `_default_branch_report` already quotes
`gh-branch`'s `Branch <ref>: <STATE>` and `Verdict: …` lines verbatim, so
the escalated state and sentence reach the merge report for free.

`presets/watch/sources/gh-branch/poller.py` treats `NO RUN — STALE` as the
same raw-empty reading as `NO RUN` for its own direction guard and streak
bookkeeping ([#2333](https://github.com/Digital-Process-Tools/claude-supertool/issues/2333))
— it is the identical zero-runs finding, only older — and gives it its own
event key, `no_run_stale`, distinct from `no_run`, so a channel consumer can
watch for the harder finding specifically.
...
## Cleanup — run by this op (`cleanup`)
  ...
  [release] 1256 — same-repo issue(s) this merge verified CLOSED; a caller with its own per-issue lane bookkeeping may release these now (#2337). This op stores nothing and calls nothing on your behalf.
```
...
- **The `[release]` line names what a caller may release, and never calls anything itself** ([#2337](https://github.com/Digital-Process-Tools/claude-supertool/issues/2337)). `cleanup` reaps the merged branch's worktree and remote branch, but a calling loop's own per-issue lane bookkeeping — `claude-oss`'s `.oss-lanes/<issue>.json` — lived entirely outside this op's knowledge, so it was never released: a stale record silently narrowed what a later dispatch-selection pass could see as free until that caller's own TTL expired. `release_candidates()` filters the exact `verdicts` list `## Linked issues` above already renders down to the same-repo issue numbers verified `CLOSED`, bare (no `#`) so a caller can consume the list directly; a cross-repo ref (`owner/repo#N`) is excluded because it never names an issue in the caller's own tracker. This is an explicit list, not a callback — the op has no concept of any one caller's lane registry and stores nothing and calls nothing on a caller's behalf, matching the issue's own steer away from growing that dependency. The line is gated on the same `MERGED` read-back every other item in this section is gated on — caught in self-review, `verdicts` reads each issue's *current* state off the live tracker independent of whether this merge attempt itself succeeded, so an unrelated closure (a duplicate, a manual close, an earlier attempt) on an unconfirmed merge must never be named as something *this* merge verified; it now prints `[release] none — the merge is not confirmed, so nothing here is attributed to it`.
...
**`body` must be a plain string, and a TOML `[[body]]` table-array is refused rather than accepted** ([#2315](https://github.com/Digital-Process-Tools/claude-supertool/issues/2315)). A payload whose `body` key parses as a TOML array-of-tables (a list of `{"value": ...}` dicts) used to reach `apply_forge_disclosure(body: str)` as a `list` with no type check anywhere upstream, crashing with a bare `TypeError` three frames deep instead of a clean refusal. `gh-issue-comment` accepted the same shape without crashing, but did not actually join it either — its own reader stringified the raw Python list, publishing the literal repr as the comment body rather than the intended prose, confirmed against a real posted comment on issue #2310. `gh-issue-create` did not copy that: a non-string `body` is refused with `ERROR: body must be a string, not TYPE -- ...`, naming `body_file` as the way to publish content from a file. `gh-issue-comment` now refuses the same shape too ([#2322](https://github.com/Digital-Process-Tools/claude-supertool/issues/2322)), with the identical message, checked both in `validate()` and again in `_body_text()` for a caller that reaches it directly.
