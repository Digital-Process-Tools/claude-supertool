...
- **`--force` was never the guarantee it was written up as** ([#1280](https://github.com/Digital-Process-Tools/claude-supertool/issues/1280)). `git worktree remove` runs without it, and that governs **modified and untracked** files — an **ignored** one is deleted regardless, and a local env file, a virtualenv or a scratch database is in no index, no stash and no remote. The tree is read with `git status --porcelain --ignored` first and refused, naming the paths, if it holds anything git is not tracking; a `git status` that could not run is also a refusal.
