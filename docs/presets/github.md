...
The marker now names both the cause and the size — `... (5 lines elided by this op — no error pattern matched them; the log itself is intact)` — and every withheld line, including any after the last shown block, is accounted for by exactly one marker. Two bugs went with it: a phantom marker was printed before the first block when the first match was line 1, covering nothing at all, and the same off-by-one sat in `:grep:`.
