...
| `gh-mirror` | `gh-mirror:issue:NUMBER` | What the write-through `gh-issue` mirror ([#1955](https://github.com/Digital-Process-Tools/claude-supertool/issues/1955)) currently holds for one issue, with **no network call**. See [The write-through mirror](#the-write-through-mirror) below. |
...
Unset (the default) means the mirror is off: `gh-issue` renders exactly as it always has, with no extra output and no extra write.

**Scope, as shipped: `gh-issue` only.** `gh-pr`, `gh-issues` and `gh-prs` are not wired to this mirror — the full lane touches four files over 4,500 combined lines, each needing its own TDD/doc/fragment pass, sized in the issue's own comment as more than one lane should land at once. Because only `gh-issue` writes through it, **`gh-mirror`'s `not-cached` means "never read via `gh-issue`", not "does not exist on the tracker."** The manifest that would let a caller tell those two apart — built from the full issue list `gh-issues` already fetches — is exactly the piece that extending the other three ops would add.

**What every `gh-issue` read stores, as a side effect:** the full, untruncated API reply (`gh issue view --json ...`), plus the timestamp of that read — never the display-capped body a non-`:full` render shows. A mirror of a truncated body would lie by omission.

**`gh-mirror:issue:NUMBER`** answers what the mirror holds, with no network call, in four renders:
