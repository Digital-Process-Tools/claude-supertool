...
**The default stays.** Changing it would silently widen the radar `gh-prs` tier's population, which keys its departure snapshot on that scope, and "my PRs" is the question the bare op is for. What changes is that the render now has three states instead of two:

| The board shows | What the footer says |
|----|----|
| rows | `author=@me (default) — your PRs, not the repo's; gh-prs:anyauthor for everyone's` |
| nothing, and rows exist without the filter | `author=@me (default) excluded 2 open PR(s) — gh-prs:anyauthor to see them` |
| nothing, and nothing is open either way | `author=@me (default) excluded none — nothing is open either way` |
| nothing, and the check could not be made | `author=@me (default) applied; whether it excluded anything is UNKNOWN — the check itself failed: ...` |

The excluded count costs one extra `gh pr list`, fired **only** when the board came back empty under the *implicit* default: an explicit `author=X` is the caller's own filter and needs no explanation, and an extra list call on every board is a real cost on a radar tick. The fourth row is the one that matters — a spawn failure is a platform difference (Windows raises `FileNotFoundError [WinError 2]` where POSIX may not fail at all, [#997](https://github.com/Digital-Process-Tools/claude-supertool/issues/997)), and reporting `excluded none` off a call that never ran would reproduce this fix's own defect class inside the fix.

`anyauthor` drops the default outright. Combining it with an explicit `author=`/`assignee=`/`reviewer=` is **refused**, not resolved by precedence: they ask for different boards, and picking one silently is the defect the flag exists to close.
