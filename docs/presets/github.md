...
## Cleanup — run by this op (`cleanup`)
  ...
  remote branch   done     deleted `fix/1256` on the remote via the API (recoverable: GitHub keeps refs/pull/N/head)
...
- **A worktree is removed only on `idle`**, read off `git-worktrees`' own exit code rather than re-derived here — `cannot tell` is treated as occupied and named, because an agent can be alive in a tree with an empty index for its first 26 minutes. `git worktree remove` runs **without `--force`**, so git's refusal on a dirty or locked tree is a second, independent gate on the one irreversible step. Note the consequence: run right after your own merge, the honest answer is usually `refused: cannot tell`, because `idle` requires an hour of quiet. The litter this exists for is day-old trees, and those pass.
