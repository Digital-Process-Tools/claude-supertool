...
This preset ships no `cmd` and no helper scripts. Enabling it does not add capability — those five ops run whether or not it is listed. What it adds is their presence in the *listing*: with the preset loaded, `ops` and `ops:full` render them; without it, they are reported under `ops`' "Also accepted, no reference in .supertool.json" line.

`help:hover` answers either way. `_shipped_config()` folds the shipped presets' `builtin-ops` into the reference behind the `help:OP` fallback, so where a built-in's documentation lives in an install is not a fact about the caller's tree — the alternative was reintroducing [#1773](https://github.com/Digital-Process-Tools/claude-supertool/issues/1773), which is `help:read` answering "no documented help" from a consumer repo.
