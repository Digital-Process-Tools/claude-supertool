...
The YouTube Data API v3's default daily quota is 10,000 units. `youtube_search` (`search.list`) costs 100 units per call -- the most expensive endpoint in the API, since full-text video search has no cheaper surface. `youtube_read` costs 3 units (`videos.list` + `commentThreads.list`). `youtube_list` costs 2 units (`channels.list` to resolve the uploads playlist, `playlistItems.list` to read it).
