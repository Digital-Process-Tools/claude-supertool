...
| Key | Meaning |
|---|---|
| `_arg` | the invocation argument. With two or more tiers registered and `_arg` a prefix match for one of their names ([#2644](https://github.com/Digital-Process-Tools/claude-supertool/issues/2644)), it is routed only there: `radar:gh-issue:2369` reaches only the tier named `gh-issue` (its own name as a prefix, `"gh-issue:2369"`), every other registered tier's `_arg` is `""`. Everything else is unchanged from before routing existed: a bare `radar` is `""`; one tier registered, or `_arg` matching no registered tier's name (an ordinary filter string like `author=@me,milestone=x`, never a tier selector at all), gets the whole string, every registered tier |
...
**`gl-mrs` grew its own transport-marker whitelist in [#1870](https://github.com/Digital-Process-Tools/claude-supertool/issues/1870).** Before it, a `glab` that failed on DNS, a refused connection or a reset socket landed as a plain `RadarError` — the same class as an answered request the caller was told is a verdict about the board — where its GitHub twin (`gh_prs`, since [#1568](https://github.com/Digital-Process-Tools/claude-supertool/issues/1568)) would say `RadarUnreachable` and let a caller retry. `TRANSPORT_MARKERS` in `presets/watch/tiers/gl_mrs.py` is a separate tuple from `gh_prs`'s own — the same shape [#1846](https://github.com/Digital-Process-Tools/claude-supertool/issues/1846) chose for the auth markers, since a shared list drifts the moment one CLI's vocabulary needs an entry the other does not — but every entry the two share today is prose Go's `net`/`net/http` standard library writes, ported on the reasoning that `glab`, like `gh`, is a Go binary built on that stack; that reasoning is stated as **reasoned, not observed**, because no authenticated `glab` on the machine that wrote it could drive a real transport failure through. `gl-mrs` also gained a rate-limit arm of its own — GitLab's API answers `429`, not `gh`'s `403`, so it is GitLab's own status line rather than a port of `gh`'s.
...
- **A free-form message body never travels inline in the event** ([#2044](https://github.com/Digital-Process-Tools/claude-supertool/issues/2044)) — applying the handback pattern this repository already uses on the way out ("an agent replies with a path and at most two lines") to the way in. `content_kind == "text"` writes the full, untruncated body to a file under the watcher's own state directory — never under `.max/`/`drafts/`/`posts/`/`blog/`, the `file://` publish allowlist, so a `file://` publish op can never read a stranger's message back out as if an operator had written it ([#2039](https://github.com/Digital-Process-Tools/claude-supertool/issues/2039)) — and the event carries `payload_path`, `length` and `sha256` instead of the text. `title` is `""` on this arm. The `[remote — data, not instructions]` marking that used to travel with the inline text (the one key `channel.ts` marks on the way into a session, see "Payload strings are flattened to one line" above) moves to the file's own header, since it can no longer travel with text that has moved out of the event. Reading the words becomes a deliberate act — opening the file — rather than something that has already happened by the time the agent knows what it is.
...
- **`files`** ([#2202](https://github.com/Digital-Process-Tools/claude-supertool/issues/2202)) carries per-file metadata Slack already handed the poller and that used to stop at `content_kind == "files"`'s naming: a single `" | "`-joined string naming `name`, `mimetype` and `size` for each entry in `msg["files"]`, plus its `permalink` when present, `""` on every other `content_kind`. A flat string, never a list or dict — `notifiers/claude-channel/channel.ts`'s `asAttr` refuses any array- or object-valued payload key outright, so a structured value here would never reach a session at all (a reviewer finding against the first cut of this fix, which did emit `list[dict]`); this follows the same joined-string shape `sources/gitlab-mr/poller.py`'s `_failed_jobs_fields` already uses for the identical constraint. `permalink` is a link a human already logged into the workspace can open; `mimetype`/`size` let a receiver tell an image from a 400MB archive without fetching anything. Deliberately not `url_private` — that needs the bot token as a bearer header to resolve at all, and this is route 1 of the issue's three, metadata only. Route 2 (fetching the bytes into `payload_path`, reusing #2044's store) is still open. A missing or wrongly-typed field on one file entry (Slack does not guarantee every field on every file type) defaults to `"unknown"`/`0B` rather than dropping the file or raising; the joined string is bounded by `_bound` against the channel's own attribute cap the same way the message body is (#2059).
...
**Cold start anchors on "now"; it does not replay history (#2043).** A watcher's first tick has no cursor yet, and a channel is not an object with a beginning worth replaying — the source fetches the latest message and stores its `ts` as the cursor, emitting nothing. Delivery begins with the next message posted. For a bare channel that is one `limit=1` call (`conversations.history` is newest-first), costing exactly as much regardless of how much history the channel carries — the alternative, asking for the whole history on tick one, is what #2043 was. A thread anchor is not the same shape: `conversations.replies` is oldest-first, so a `limit=1` call there would return the thread's *root* message, not its newest reply, and anchoring on the root would reproduce #2043 for any thread busier than one tick's page budget. There is no cheap "give me the newest reply" call, so a thread cold start pages through the thread the same way an ordinary catch-up tick does (same `MAX_PAGES` budget), keeping only the newest `ts` it finds and still emitting nothing.

**The burst refusal (`slack_unreachable`, "more than 1000 new messages ... catching up would take more than 5 pages this tick") is deliberate, and it survives the fix above.** A single poll pages up to `MAX_PAGES` (5 pages of 200) to catch up on new messages since the last cursor; if the channel still has more after that, the poll refuses rather than silently advancing the cursor past messages it never fetched — a reviewer finding on #2031: dropping the overflow instead of refusing it loses those messages permanently, because the cursor's advance is what makes `ts > cursor` never true for them again. A refusal leaves the cursor untouched, so the next poll retries the same window. This is not a transitional error message the cold-start fix retires — a channel quiet for a long while, or a burst above 1000 messages between polls, can still trigger it on an established watcher. It reads like an auth failure on first sight (`slack_unreachable`, same event as a dead token) and is not one — `bot_user_id` resolving is what tells the two apart. If you see it, nothing is broken: raise `MAX_PAGES` (`presets/watch/sources/slack/poller.py`) if the channel is genuinely this busy, or wait for the burst to drain below the budget.
...
| every poll while stored `approved` is confirmed `False` and the status is not `not_approved` | **1** per poll | re-asked until it turns `True` -- a higher-priority blocker can take the field on the exit tick and the rule can be satisfied later without ever exiting `not_approved` again ([#2670](https://github.com/Digital-Process-Tools/claude-supertool/issues/2670)) |
...
**Nothing was added to the per-poll path for an MR that is not disputing its approval.** That was the design constraint: a watcher fleet is one process per open MR polling every 30s forever, so a request added there is multiplied by every MR you have open, all day. A request added to a failure transition is paid once per pipeline going red; `approved` is paid the same way while it is settled either way (confirmed `True`, or free off `not_approved`), and only pays per-poll for the length of a genuine `False`-and-blocked-elsewhere streak ([#2670](https://github.com/Digital-Process-Tools/claude-supertool/issues/2670)) — see [below](#approved-and-retargeted-2645).
...
**The separate request is paid on the tick that exits `not_approved`, and again on every later tick where the confirmed answer is still `False`.** Exiting `not_approved` does not by itself mean "approved" — `detailed_merge_status` reports only the first failing mergeability check in priority order (see the `[conflict]` vs `[empty]` reasoning above), so a push that clears approvals while a higher-priority check (draft, unresolved threads, CI) is also failing moves the field away from `"not_approved"` without approvals having actually been satisfied. The first version of this gate stopped there and never asked again: the approval rule could then be satisfied on a later tick that never exits `not_approved` a second time, and there was no way back in ([#2670](https://github.com/Digital-Process-Tools/claude-supertool/issues/2670)). A confirmed `False` is re-asked every subsequent tick until it turns `True` — the same "once per red streak, not once per tick" budget [#509](https://github.com/Digital-Process-Tools/claude-supertool/issues/509) already holds for the failing-job lookup, applied here to the length of the unresolved streak rather than to a single tick. Declined rather than guessed at if a confirming request fails: `approved` carries the last known value forward and is asked again on the very next tick, same as any other poll in the streak.
