...
**Cold start anchors on "now"; it does not replay history (#2043).** A watcher's first tick has no cursor yet, and a channel is not an object with a beginning worth replaying — the source fetches the channel's single latest message (`limit=1`), stores its `ts` as the cursor, and emits nothing. Delivery begins with the next message posted. This costs one cheap call regardless of how much history the channel carries, which is the point: the alternative — asking for the whole history on tick one — is what #2043 was.
