...
| `gl-job` | `gl-job:NUMBER[:raw[:-N\|:START[:END]]\|:grep:PATTERN]` \| `gl-job:NUMBER[,NUMBER...]:trace` | Job failure detail: MR context + error pattern search + log tail. `:raw` dumps the full trace; `:raw:START:END` slices lines (1-indexed, inclusive); `:raw:-N` returns the **last N lines**; a START past the end of the log returns the tail of the width requested and says so, rather than declining (see [Reading a range](#reading-a-range)); `:grep:PATTERN` runs an ad-hoc regex over the trace (literal fallback on bad regex, ±context, names the pattern + tail on no-match — never silent-empty). Cause markers are always matched on top of the configured patterns, and a *failed* job that matches nothing is reported as unclassified with a log tail rather than as "no errors". `:trace` ([#626](https://github.com/Digital-Process-Tools/claude-supertool/issues/626)) writes the whole trace to a file under the process's own proven-ours temp root, never into context — see [Writing a trace to disk](#writing-a-trace-to-disk) |
