...
Nothing was pushed. Name the target once:
  git push -u origin HEAD          # push fix/x under its own name (the usual first push)
  git push origin HEAD:master   # push onto master on purpose, if that's the real target
```
