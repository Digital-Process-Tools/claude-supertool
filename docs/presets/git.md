...
| `git diff --shortstat` against the MR/PR target timed out, or the target ref wasn't fetched locally | bare `Diff: N files` — identical to a genuinely empty diff | `Diff: N files (+? -? — 'git diff --shortstat' did not answer: <reason>)`; a timeout also lands in the footer above |
