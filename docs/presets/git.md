...
| `git-worktrees` | `git-worktrees[:PATH][:nopr][:since=SECONDS\|:since=@EPOCH]` | **Is an agent working in this worktree?** Every worktree (or one `PATH`) with branch, path, a merge state in **four** states (`merged` / `not merged` / `no commits yet` / `merge unknown`, each naming the method — ancestry cannot see a squash merge, and it cannot tell a landed branch from one that never committed, see [The merge column](#the-merge-column-has-three-states-and-ancestry-answers-only-one-of-them)), an **uncommitted-work** state (`clean` / `dirty: N` / `dirty unknown`) which is the only column a detached tree can answer, and an occupancy verdict in **three states** — `occupied`, `idle`, `cannot tell` — each naming the evidence it was built from. `cannot tell` is the honest majority answer and is **not** `idle`; see [Occupancy has three states](#occupancy-has-three-states-and-idle-is-the-one-that-must-be-earned). Inspection only: nothing is removed, pruned, unlocked or written |
...
**A MESSAGE holding a raw control byte is refused before anything is staged** ([#2592](https://github.com/Digital-Process-Tools/claude-supertool/issues/2592)). Lane fix/2573 hit `git-commit:::MESSAGE:::PATH` refusing with `no PATHS were given` despite a PATH given inline, twice — and one of the two real commit messages carried a genuine embedded ESC (0x1B) byte. Fed straight through this repo's own argv-list parser with no shell involved, that exact message-plus-path combination commits cleanly, so the byte is not lost here; the leading theory is a shell quoting form (`$'...'`) turning escape TEXT into the real byte, with an interactive pty's own readline then reading it as a control sequence and dropping the rest of the line before this script's argv ever forms — upstream of anything this repo controls. What is in reach: catching the same hazardous byte (an ESC or other C0/DEL/C1 control, or a Unicode line/paragraph separator — the last two arrive as themselves, a lone C1 byte only as the surrogate `surrogateescape` produces from it, since a standalone C1 byte is not valid UTF-8) the next time a call carries it whole, naming it and its position before the repo probe or anything else runs, on both the colon-CLI and `@file` routes.
...
**A caller who knows better can say so** ([#2272](https://github.com/Digital-Process-Tools/claude-supertool/issues/2272)). The gap window above is exactly where a caller's own `git merge`/`git push` lands: reproduced live on a sibling repo, a maintainer merged `origin/main` into a lane's worktree itself — no agent involved — and a readiness read of that same tree moments later returned `cannot tell`, citing the maintainer's *own* index write as the disqualifying signal. `cannot tell` is not `idle` and the downstream rule (never brief a second agent into that tree) was followed exactly, so the result was a blocked dispatch for a reason that had nothing to do with actual risk — the op had no way to hear "I just wrote this myself".

`since=SECONDS` (or `since=@EPOCH` for an absolute timestamp) declares a known-good cutoff for the one tree named by `PATH`: any write at or before that point is attributed to the caller and excluded from the occupancy signal, rather than read as evidence of a live agent. A write **strictly after** the declared cutoff is untouched — the declaration cancels evidence at or before the instant it names, never the whole signal, which is what stops a real agent's write landing a moment later from being swallowed along with the caller's own. The declaration is always disclosed in the evidence (`known-good declaration: writes at or before …`), whether or not it changed the verdict, so it is never applied silently. `since=` without a `PATH` is refused — the declaration is about one specific tree, and there is no single cutoff to apply across an entire board of independent worktrees. **A `PATH` that resolves to more than one row is refused the same way**: the filter (see [A `PATH` that matches more than one worktree exits `2`, not `0`](#occupancy-has-three-states-and-idle-is-the-one-that-must-be-earned) above) is ancestor-or-descendant, so a nested worktree layout can match the tree above or below the one named — applying one caller's declaration to every match would launder occupancy evidence for a worktree the caller never touched. **Two DISAGREEING `since=` tokens on the same call are refused too**, on the same terms `git-push:budget=SECONDS` already uses for two disagreeing `budget=` tokens: a repeated but identical value is the same claim typed twice and is accepted, but two different values are two different claims about when the tree was written, and this declines to silently keep the last one.

This is deliberately a narrower fix than a general "richer reasons" pass would have been: the existing `cannot tell` reasons already name *which* probe went silent (the cwd scan, or the write-recency check), and that granularity was not the reported gap. The reported gap was that no caller-held fact could ever change the verdict, however precisely the tool explained itself — a maintainer who just merged and pushed still could not get past `cannot tell` by explaining why. `since=` closes that loop directly rather than by making the prose more informative.
...
**`supertool`'s own exit follows the same rule, and for one release it did not** ([#1705](https://github.com/Digital-Process-Tools/claude-supertool/issues/1705)). The registry declares these three integers `exitStatus` values, so supertool counts the call as an answer rather than a refusal (#1672) — but it also began exiting `0` for `occupied` and for `cannot tell`, and `0` is the value every shell guard tests. It now declares `"clean": [0]`, so supertool exits `0` only for `idle` and non-zero for the other two. The op's own integer is still not the process exit: supertool collapses to `0` or `1`, so a shell that needs to tell `1` from `2` has to run `presets/git/worktrees.py` directly.
...
**The tracker column stays out of the exit code**, permanently: a tracker that could not be read says nothing about whether a tree is safe to enter, and folding it in would make `git-worktrees:PATH` refuse a free worktree because GitHub was down. That is a rule about *network* columns, and [#1751](https://github.com/Digital-Process-Tools/claude-supertool/issues/1751) did not weaken it — the uncommitted-work column is a **local** read about the tree itself, and it is in the integer ([below](#the-exit-code-now-answers-safe-to-reap-and-exit-0-got-stricter)).

## The merge column has three states, and ancestry answers only one of them
<!-- Heading kept verbatim so the anchor below stays valid; the column gained a
     fourth state in #1750 and the table under it is the current list. -->
...
| token | what it means |
|---|---|
| `merged` | an ancestor of the base **whose reflog shows it did commit**, or a merged PR whose head is this branch — the evidence line says which |
| `not merged` | both were consulted and neither said yes |
| `no commits yet` | the branch has never moved since it was created, so there is nothing to measure ([#1750](https://github.com/Digital-Process-Tools/claude-supertool/issues/1750), below) |
| `merge unknown` | something could not answer, and the line names which: offline, `nopr`, the lookup failed, the merged-PR page hit its cap, or the reflog did not answer |
...
### A branch with no commits is an ancestor by holding nothing

Observed on the live board at `62bc3f0` ([#1750](https://github.com/Digital-Process-Tools/claude-supertool/issues/1750)): **seven of eight** fleet worktrees rendered `[merged]` while an agent was editing each of them seconds earlier. None had committed anything — the developer agents commit once, at the end. `merged: yes — every commit is already an ancestor of master` is **vacuously true of a branch with zero commits**, so a brand-new tree and a fully-landed one were the same cell, in the column a maintainer reads to decide what is safe to delete.

The occupancy column happened to save those seven — they read `occupied`, so a gated reap declined. But that holds only while the heuristic fires. An agent quiet past the activity window drops to `idle` while its branch still says `[merged]`, and *idle and merged* is the exact signature of a tree to reap.

**The issue's own proposed test could not have worked, and this is worth knowing before you reach for it.** `git rev-list <base>..<branch>` being empty is not a second opinion — it is the *same predicate* the ancestry read already performs, because a branch is an ancestor of the base **iff** that range is empty. Measured on git 2.46.2:

```text
                       for-each-ref --merged master : feat/real, fix/new, master
       rev-list --count master..feat/real (3 commits, merged --no-ff) : 0
       rev-list --count master..fix/new   (created, never committed)  : 0
```

The commit graph cannot separate *landed* from *never started*: both leave the branch tip reachable from the base. The branch's **reflog** can, it is local, and it costs nothing on the network, so `nopr` keeps answering:
...
So ancestry keeps its `merged` and gains a precondition. It is still read as a **positive only** and still never as a `no`; what changed is that the positive now requires evidence the branch ever held commits of its own. A log holding nothing but `branch: Created from …` renders `no commits yet`. A log that **could not be read** — `core.logAllRefUpdates` off, a bare repo, a fresh clone that did not create the ref locally, expiry that trimmed the oldest entries — is `merge unknown`, naming the reflog. It is never a fall-through to `merged`, because answering `False` there would render `no commits yet` over a branch that may hold a year of work.

The read is `_git_verbatim`, not `_git`: `%gs` is a reflog **subject**, which for a commit entry is somebody's commit message, and `_git` runs `Popen(text=True)`, which rewrites a lone CR and a CRLF to LF before any preset sees the stream. With the translation off a reflog line cannot contain LF by definition.

## Does this tree hold work that exists nowhere else?

`git-worktrees` reported a detached tree as safe to reap ([#1751](https://github.com/Digital-Process-Tools/claude-supertool/issues/1751)), and it was right about everything it said:

```text
idle    (detached)  /…/scratchpad/prevwt  PR n/a
        · no process has its cwd inside this worktree — 521 processes scanned via lsof
        · merged: n/a — no branch checked out here (detached or bare), so there is
          nothing to measure against master
[exit 0] the occupancy verdict … is `idle`
```

A detached HEAD has no branch, so the merge column is `n/a` **correctly** — and that made `idle` the entire verdict standing between `git worktree remove` and destroyed work. `idle` is a statement about the process table at one instant, which the op's own footer says. The remaining question is one line, and four routes each answered a different one: `git -C W status --porcelain` is blocked by the `git-C-has-cwd` rule, whose substitution table sent the reader to `git-worktrees`; `cwd:` + `git-status` is refused by the mixed-tree guard when the target is itself a supertool checkout; the target tree's own `supertool.py` is a different repository's core.

Every row now carries the answer, in three states, branch or not:

| token | what it means |
|---|---|
| `clean` | `git status --porcelain` was read in that tree and returned no records |
| `dirty: N` | N change **records** — this tree holds work that exists nowhere else |
| `dirty unknown` | the read did not answer (timeout, missing directory, git failed), naming why. **Never `clean`** |

`clean` prints out loud rather than being the absence of a tag, for [#1229](https://github.com/Digital-Process-Tools/claude-supertool/issues/1229)'s reason one column over: a row carrying nothing would mean both *clean* and *the column did not run*. The dirty and dirty-unknown counts ride the `[result]` line, so they survive `| tail -1`.

Two implementation choices that are not cosmetic:

- **`-z`.** The count is printed and a filename may contain a newline, so NUL-separated records make it exact rather than inflatable by whoever named the file. A rename carries its origin path as its **own** NUL field (measured, git 2.46.2: `R  new.py<NUL>old.py<NUL>`), so counting fields would report two changes for one renamed file. An untracked directory collapses to one record (`?? udir/`) — that is git's own summary, not a loss this layer introduces.
- **`--no-optional-locks`.** `git status` otherwise refreshes the index and writes it back. This op's contract is INSPECTION ONLY: it must not write inside a tree a live agent is holding, must not contend for `index.lock`, and must not perturb the newest-write mtime the occupancy column one cell to the left is reading.

### The exit code now answers *safe to reap*, and exit 0 got stricter

The integer was a statement about occupancy alone. It now covers uncommitted work too, and the change makes **fewer** trees qualify for `0`, never more:

| code | meaning | `clean` to supertool? |
|---|---|---|
| `0` | idle **and** clean | yes |
| `1` | occupied | no |
| `2` | cannot tell — including a dirty scan that did not answer | no |
| `3` | idle, but holding uncommitted work ([#1751](https://github.com/Digital-Process-Tools/claude-supertool/issues/1751)) | no |

The **tracker** column stays out of this integer, and that reasoning is unchanged: a lookup that did not answer says nothing about whether the tree is safe to enter, and folding it in would make `git-worktrees:PATH` refuse a free worktree because GitHub was down. The **dirty** column is in it because it is a *local* read about the tree itself — the same class as occupancy, not the same class as the network columns — and it is the one fact the gated call destroys.

`3` is declared in `exitStatus.values` and **not** in `clean`, so `supertool 'git-worktrees:PATH' && <reap>` declines over a dirty tree. Through supertool the integer still collapses to 0 or 1, so `2` and `3` are indistinguishable there — which is the point, since all three mean do-not-reap. A shell that wants to branch on the integer runs `presets/git/worktrees.py` directly. A caller testing `== 1` for `occupied` is untouched: occupancy keeps its own code and still outranks this one.
...
**The cap does not hide the row the column was added for.** git lists untracked paths alphabetically, so a stray file can sort past the default view's ten. Every row is stat'd, including the cut ones, so the truncation marker carries the newest write among them — `... (5 more, newest of them written 12s ago)` — rather than costing a second call to `git-status:full`.

Three states there too, because the first draft of that line had two: a bare `... (5 more)` renders identically whether every hidden row was stat'd and none was recent, or **none of them could be stat'd at all**, which is precisely the state another process writing in your tree produces. So a tail nobody could time says `... (5 more, newest write among them UNKNOWN — no mtime here could be read)`, and a partly-readable tail says how much of it it is speaking for: `... (5 more, newest of them written 12s ago; 2 of 5 mtimes unreadable, so a newer write may be among them)`.
...
| `git diff --shortstat` against the MR/PR target timed out (the target ref not being fetched locally is disclosed on the same line, but stays out of this footer — see below) | bare `Diff: N files` — identical to a genuinely empty diff | ``Diff: N files (+? -? — `git diff --shortstat` did not answer: <reason>)``; only the timeout lands in the footer above |
...
**The MR diff line had its own two-state gate until [#1933](https://github.com/Digital-Process-Tools/claude-supertool/issues/1933).** `changes_count` from the tracker's own JSON already proves the diff is non-empty before `git diff --shortstat` is ever run, so a bare `Diff: N files` inside that branch could only mean the local call did not answer — never that there was genuinely nothing to count. The gate used to be `returncode == 0 and stdout.strip()`, so a timeout, an unfetched target ref, and any other git failure all rendered identically to that impossible "empty diff" reading. The comment above the code had called the silent fallback deliberate; [#1913](https://github.com/Digital-Process-Tools/claude-supertool/issues/1913) then named the vanishing `(+a -d)` figure as the harm it was fixing one call earlier in the same block, and the two could not both be current. Settled here: not deliberate — but disclosure happens **on this line only**, not through `_note_failed`/`_UNANSWERED`, because an unfetched target ref is the ordinary case on a shallow or partial checkout and `_note_failed`'s own contract is to stay silent on a call that "fails as a matter of course". A clean, non-empty result still renders `(+a -d)`; an empty result renders `(+0 -0)` rather than nothing (the diff being genuinely empty on a real repository is possible even when `changes_count` is non-zero, e.g. a rename with no content change); a target ref the tracker never reported at all renders `(+? -? — no target branch reported for this MR/PR)`; and any local git failure renders `` (+? -? — `git diff --shortstat` did not answer: <reason>) ``. Only a genuine timeout also reaches the `INCOMPLETE` footer, because `_git` itself records `TIMEOUT_RC` unconditionally regardless of this line.

**The `INCOMPLETE` footer itself could carry the same forged line break, one hop later.** [#1939](https://github.com/Digital-Process-Tools/claude-supertool/issues/1939): `_git()`'s generic timeout path records `("git " + " ".join(args), res.stderr)` in `_UNANSWERED`, and one of `_git()`'s 17 call sites is the shortstat call above, whose ref is built from `mr_target_raw` — the target branch exactly as the tracker reported it, unflattened, because the ref has to be built from the raw name (see "The target branch on that line is the opener's text" below). `Target:` already flattens that value for its own render (#977); the timeout-recording path did not, so a target branch carrying U+2028 or U+2029, forced to time out, put the raw separator straight into the footer. Fixed at the one append `_git()` owns rather than at each of the 17 call sites or inside `_incomplete_note()` at render — the seam every timeout passes through regardless of caller, and safe against a value that already went through `_untrusted.flat()` upstream because `flat()` is idempotent.
...
**The detector is a ratchet, and that is a decision rather than a shortcut.** An AST scan for "a child stream reaches a render with no marker between" reports **144 sites in 32 files** across `presets/git`, `presets/github`, `presets/gitlab` and `presets/_git_run.py`, which is named on its own because [#2447](https://github.com/Digital-Process-Tools/claude-supertool/issues/2447) moved the git invocation chokepoint to the `presets/` root and took two of `_git_common.py`'s sites with it — the total did not move, the sweep just had to be told where to look, and until it was it read the two as fixed. Not all are defects: `push._local_head` returns `r.stdout.strip()` and that is a SHA. So `tests/test_forged_child_stream_line_1475.py` records a count per file that may only fall.
