...
The read is `_git_verbatim`, not `_git`: `%gs` is a reflog **subject**, which for a commit entry is somebody's commit message, and `_git` runs `subprocess.run(text=True)`, which rewrites a lone CR and a CRLF to LF before any preset sees the stream. With the translation off a reflog line cannot contain LF by definition.
