...
**The detector is a ratchet, and that is a decision rather than a shortcut.** An AST scan for "a child stream reaches a render with no marker between" reports **171 sites in 38 files** across `presets/git`, `presets/github` and `presets/gitlab`. Not all are defects: `push._local_head` returns `r.stdout.strip()` and that is a SHA. So `tests/test_forged_child_stream_line_1475.py` records a count per file that may only fall.
