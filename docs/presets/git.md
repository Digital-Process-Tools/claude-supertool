...
**The detector is a ratchet, and that is a decision rather than a shortcut.** An AST scan for "a child stream reaches a render with no marker between" reports **156 sites in 34 files** across `presets/git`, `presets/github` and `presets/gitlab`. Not all are defects: `push._local_head` returns `r.stdout.strip()` and that is a SHA. So `tests/test_forged_child_stream_line_1475.py` records a count per file that may only fall.
...
The rest are not defects, and most of them are safe on grounds just as good: a forged row can only refuse (`_list_conflicts`), or is fail-closed (`_union_attr_paths`), or inflates a count in its loud direction (`_discarded_by_force`), or its writer is the local operator (`_remotes_could_host_a_request`). Each entry now opens with the ground it rests on — `QUOTED PATH`, `NOT QUOTED, harmless`, `NOT QUOTED, open` — and the register refuses one that names none.
...
The 38 remaining sites across 27 enclosing functions are each registered with the ground they rest on, in `tests/test_preset_git_splitlines_register_1130.py`. A new `str.splitlines()` anywhere under `presets/git/` is a red build until someone writes down which kind it is — and, since #1654, until they name one of the three grounds rather than inheriting a rule that answered for three entries out of twenty-seven.
