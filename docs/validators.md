...
So is one input the adapter cannot read: a `<script` start tag that is never closed — an attribute value whose quote never closes, or a file that stops mid-tag. Both are EOF-in-tag, where the tokenizer drops the tag outright. Nothing in the file says where that tag ends, or whether it has a body, so the adapter declines, naming the line and which of the two it met, rather than cutting the tag at a `>` that is inside a string. Regex-scanning markup has a floor, and the honest move at that floor is to say so.

The refusal is **per file, and it outranks findings already in hand**. A page with a broken block at line 3 and an undelimitable tag at line 40 reports `skipped`, not `ok: false, count: 1` — the latter is a verdict about a file that was never read past line 40, and this adapter does not make claims it cannot support. Nothing is lost: the reason names the line to fix first, and the finding is still there on the next run.
