...
- **The budget is `SUPERTOOL_GIT_TIMEOUT`, the same variable the op above reads, and the two defaults differ on purpose**: 5s covers *one* call in `presets/git/status.py`, 15s covers *all four* in the adapter. Export it and you move both, which is the intended behaviour — a repository slow enough to need one is slow enough to need the other. Values below `1`, and anything unparseable, fall back silently: `presets/_env.env_int` announces a bad value on **stdout**, and the core parses this adapter's stdout as JSON, so the helper that is right for a preset would turn a working validator into `no_verdict`.
