...
| Markdown            | `markdownlint`   | `markdownlint` CLI                 | `npm install -g markdownlint-cli`. Defers to `changelog-fragment` for a `*changelog.d/*.md` path when the project's own assembler script is found above it -- `skipped`, not `ok`, so a project's own no-heading/no-wrap fragment convention is not read as malformed prose (#2338); see the changelog-fragment row below |
...
| Git           | `git-status`     | `git` binary                                      | Reports working-tree delta (`+N -N <state>`) post-edit. Budget `SUPERTOOL_GIT_TIMEOUT` (default **15s**, for the whole adapter). A git that outruns it declines `NOT CHECKED` rather than reporting `clean`. See [README](../validators/git-status/README.md) |
...
| GitLab CI config  | `ci-lint`       | `glab` on PATH, authenticated       | `glab ci lint` against the resolved repo-root `.gitlab-ci.yml` (never the edited include, which never validates alone — mapped by this entry's own `resolve` command, `validators/common/ci_lint_resolve_root.py`). Matches `.gitlab-ci.yml`; the sibling `ci-lint-includes` entry matches `.gitlab/ci/*.yml` and resolves to the same root. A network/auth failure — glab could not confirm the config with GitLab — reports `skipped`, never `ok: false`, so `rollback_on_fail: true` cannot mistake an outage for an invalid config (#1797) |
...
**`NOT CHECKED` is the same absence arriving through the error channel.** A `skipped` is a checker that declined *before* running and had a field to say so with. An adapter that was asked to run, could not, and holds only an `adapter`-coded error has produced exactly as little information — and every consumer downstream of a result treats an error as a measurement of the file. So a result whose errors are *all* `code: "adapter"` is rendered as `NOT CHECKED`, is never subtracted from a baseline, and is named on the `[result]` line. All of them, not the first: an adapter reporting four real findings alongside one adapter row has still measured the file, and hiding that would be this defect pointing the other way. That is a rule about *rendering*; the arithmetic below exempts the row rather than the payload, and the two answers differ for exactly the mixed case.

**An exception escaping `main` is the same absence with the diagnosis thrown away, and it is the one route the adapter itself has to close** ([#1697](https://github.com/Digital-Process-Tools/claude-supertool/issues/1697)). An adapter that dies writes **nothing** to stdout. The core has read that correctly since [#634](https://github.com/Digital-Process-Tools/claude-supertool/issues/634) — `if not out` precedes the parse, and the reply is a `skipped` saying `produced no output` — so this was never the false clean it is easy to assume. What it was is worse to debug and quieter than it should have been: the traceback goes to **stderr**, which `_validator_run_one` captures and never reads, so the operator was told a checker died and never told why; and because that skip is one the *core* produced, it escalated only for a validator named in `$SUPERTOOL_REQUIRE_VALIDATORS`. Measured end to end on a `.json` file holding invalid UTF-8, `jsonlint` — whose `open(..., encoding="utf-8")` raises `UnicodeDecodeError` before any handler it owns — printed `skipped — jsonlint adapter produced no output` and **exited 0**, so `supertool 'edit:x.json' && git commit` committed on a checker that had crashed.

`validators/common/refusal.guard_main(TOOL, main)` is the fix, wrapped around the call in every adapter's `if __name__ == "__main__"` block, and it publishes `refusal.crashed()`: `ok: false, count: 1`, one `code: "adapter"` row naming the exception class, its message, and the last three lines of the traceback, with a measured `duration_ms`. The same file now reports `NOT CHECKED — adapter jsonlint adapter crashed and did NOT check this file: UnicodeDecodeError: 'utf-8' codec can't decode byte 0xff in position 7` and **exits 1**.

Three decisions inside that are worth stating, because each has an obvious wrong answer:

- **`code: "adapter"`, not `skipped()`.** The third state is the right *category* and `skipped` is the wrong *word* for it, for a mechanical reason: an adapter cannot set `no_verdict` (a core-only key, stripped on the way in), and `_validator_no_verdict` returns `None` the instant `skipped` is a key. A crash spelled `skipped` would therefore reach neither `_note_not_checked` nor `_NOT_CHECKED` and would exit **0** — quieter than the bare crash it replaced. A crash net that lowers the alarm is worse than no crash net. The `adapter` code is the third state arriving through the error channel, exactly as the `NOT CHECKED` paragraph above describes.
- **At the call site, not inside `main`.** Of the five nets that existed, only `ruby-check`'s wrapped the whole callee; the four MCP adapters wrapped `ensure_daemon` + `ndjson_call` and left their own `print(json.dumps(format_response(...)))` outside every handler they had. So the number of adapters that could reach the process boundary with stdout empty was **35 of 36**, not the 31 with no `except Exception` anywhere — a handler that exists is not a handler that covers.
- **The exception class is published even when the exception says nothing.** `ruby-check`'s net emitted `str(exc)` alone, and `str(KeyError())`, `str(RecursionError())` and `str(SystemError())` are all empty, so its most likely output was a row with a blank reason.

`tests/test_adapter_crash_net_1697.py` drives all 36 adapters as `__main__` in a subprocess with a failure injected into their publish path, and pairs every must-not-be-silent case with a must-fire one — an uninjected run of the same adapter, and a real `jsonlint` finding that has to survive the net rather than be eaten by it.

**A timeout is the fourth route to the same absence, and it needs no config to happen.** When the core's own budget expires it writes `ok: false, count: 1` with an `orchestrator` code — the same fabricated count, reached by a slow machine rather than a missing binary. It is now rendered `NOT CHECKED (timed out — no verdict about this file)`, is excluded from the delta, and exits non-zero, where it used to read `1 err (timeout)`: a count about a file the checker never finished reading.
...
**A mixed payload is a verdict and a stall at once, and the two halves are read by different questions** ([#1717](https://github.com/Digital-Process-Tools/claude-supertool/issues/1717)). One paragraph up, the `NOT CHECKED` test is `all(code == "adapter")`, and it is the right test for *rendering*: four findings beside one stall row means the file was measured, and printing `NOT CHECKED` over them would be the absence read as a pass. It was also the only thing standing between that payload and the arithmetic, so a mixed result fell straight through to it — one finding before, the same finding plus a stall row after, delta `+1`, and on a `rollback_on_fail` validator the correct edit was written back to its pre-edit bytes. `cargo-check` ships this shape on `master`: a crate diagnostic naming another file keeps its text and takes `code: "adapter"` (`SCHEMA.md`, "A located diagnostic still has to be about *this* file"), beside the real findings about the file under validation. The subtraction now drops the `adapter` rows and keeps the rest — `_validator_measured_count`, on both sides, because a stall in the *baseline* must not cancel a real finding either. The rows are still printed with their codes; they are only out of the count, and a stall row is no longer bulleted `+` as something the edit introduced. The subtraction has a floor at the number of non-`adapter` rows visible in `errors`, because `count` and `errors` have independent sources — twenty adapters write `count = len(errors)`, `phpstan` takes it from `totals.file_errors` — and an adapter whose `count` already excluded its stall rows would be subtracted from twice, collapse to zero on both sides, and leave the gate inert over a real new finding. Correcting a count is in scope; contradicting the rows printed under it is not. `orchestrator` is deliberately not dropped per row: the core's own timeout arrives as a whole fabricated payload carrying `timeout`, which is caught before any arithmetic runs, so an `orchestrator` row sitting beside a finding is an adapter writing the core's provenance code and is not honoured (§"Core-only fields" is the same boundary at the other door).

**The floor under that subtraction has a hole, and no arithmetic closes it** ([#1728](https://github.com/Digital-Process-Tools/claude-supertool/issues/1728)). `max(count - absences, len(rows) - absences, 0)` serves two conventions at once: the floor stops an adapter whose `count` already excluded its stall rows being subtracted from twice, and subtracting from `count` rather than recomputing from `errors` stops a capped list reading fifty findings as five. Neither term survives a payload whose `count` is bounded by **the same cap** that bounds `errors` — `count=4` over `[f1, f2, f3, f4, stall]` before, `count=4` over `[f5, f2, f3, f4, stall]` after: both sides measure 4, `_validator_regressed` returns false, and a `rollback_on_fail` validator does not revert over a genuinely new finding. Every candidate arithmetic trades one blind spot for the other, because **a cap is invisible in a single payload unless the adapter declares it**. So the fix is a contract, not a formula: `count_basis` (`total` or `measured`) and `errors_truncated` say which convention the payload uses, an adapter may pre-subtract **or** cap and never both, and `count` must never be truncated by the cap or fall below the rows printed under it. A declaration contradicting its own rows is published as an `adapter`-coded fault — `NOT CHECKED`, out of the delta, no rollback, non-zero exit — rather than a comparison that cannot mean anything; never `skipped`, which would be quieter than the defect it reports. Declaring is optional at runtime, because a repo's own validator in `.supertool.json` is part of this population and a runtime mandate would break every third-party adapter on upgrade; it is mandatory for a shipped one, held by a grandfathered set that may only shrink. Two limits are stated rather than papered over: declaring changes **no number** for a conforming payload — the guard is the whole benefit — and an adapter that caps while declaring `measured` is indistinguishable, in one payload, from a well-formed `measured` one. `validators/SCHEMA.md`, "Declaring how `count` relates to `errors`", is the contract; `tests/test_count_basis_contract_1728.py` pairs every must-be-refused payload with a conforming one that must still measure, and drives both through `supertool.main()` on a `rollback_on_fail` validator so the post-condition is the file's bytes.
...
**The variable gates exactly one thing: a checker the operator named that the *core watched break down*.** That is `_validator_gate_did_not_run` — an adapter that produced no output, non-JSON output, or a reply with no verdict key. Those become a `skipped`, and a `skipped` from an unnamed validator stays quiet, because escalating it unconditionally would fail every unrelated edit on a laptop that never installed an optional tool ([#665](https://github.com/Digital-Process-Tools/claude-supertool/issues/665)). A **crash** used to be on that list and is not any more: since [#1697](https://github.com/Digital-Process-Tools/claude-supertool/issues/1697) the adapter catches it and self-reports, so it arrives as a `code: "adapter"` error and escalates through the row above whatever the variable says.

**Three absences escalate whatever the variable says**, because `_note_not_checked` records them through `_validator_no_verdict`, which never consults it:
...
| the adapter crashed — an exception escaped `main` | `NOT CHECKED (<class>: <message>)` | **exits 1** |
| the adapter broke down — no output, non-JSON, no verdict key | `skipped` | exits 0 unless named |
| the adapter declined on its own terms — `warm_unsafe`, daemon absent, unsupported platform | `skipped` | exits 0, never escalates |

The first three are the same fact seen from different sides of the same pipe: a checker that was asked about this file and produced no opinion about it. The adapter was healthy enough to say so in the first three rows and not in the fourth, which is the whole distinction — a self-report the core can trust, versus a breakdown where the core has only its own observation and the operator's stated expectations to go on. A timeout in particular needs no configuration at all to happen: the core did not decline to look, it ran out of time doing so, and nobody opted into that.
...
**The `git-status` *validator* was the last copy of that collapse, and the most expensive** ([#1882](https://github.com/Digital-Process-Tools/claude-supertool/issues/1882)). Everything above is about the `git-status` **op**; the adapter under `validators/git-status/` is a different file, and it wrapped each of its four git calls in a hard-coded `timeout=5` and returned `""` on `TimeoutExpired`. Its own parsers read `""` as an answer — `_parse_state("")` is `"clean"`, `_parse_numstat("")` is `(0, 0)` — so a repository too slow to reply published `ok: true` with a zeroed `metrics` block reading `state: "clean"`: the exact shape [#1202](https://github.com/Digital-Process-Tools/claude-supertool/issues/1202) had already removed from this adapter's *absent-git* arm, surviving on the line next to it, and firing on **every edit** rather than only where git was missing. It now declines with `code: "adapter"` — `NOT CHECKED`, never cached, never able to roll an edit back — by the rule two paragraphs down: the binary was found and the process started, so this is a `1 err` and not a `skipped`.

Three things came with it, and each is a fact a reader can trip over.

- **The budget is `SUPERTOOL_GIT_TIMEOUT`, one env var read at three sites with three different defaults, and that is on purpose, not a drift** (#1886 — this list used to say "the two defaults", missing the third entirely). `presets/git/status.py` gives its own courtesy calls **5s** — a report the caller wants back fast, and none of its calls write anything. Six other presets (`git-conflicts`, `git-worktrees`, and friends) share `presets/git/_git_common.git_timeout()`'s **10s**, the value most of them already used before #650 consolidated ten separate copies into one. This adapter's **15s** sits above both: a `git status` on a large cold repository was measured over 15s once (#1882), and this budget has to survive four sequential calls rather than one. Export the variable and you move all three together, which is the intended behaviour — a repository slow enough to need one of these budgets is slow enough to need the others — but the three numbers are not one default wearing three names: each was chosen for its own call shape and justified inline at its own site, and unifying them is a decision about three call sites' interactive-latency budgets that needs a cold-repository cost nobody has measured yet, not a docs fix. Values below `1`, and anything unparseable, fall back silently at every site: `presets/_env.env_int` announces a bad value on **stdout** for the two presets, and the validator does the same silently for its own reason — the core parses this adapter's stdout as JSON, so the helper that is right for a preset would turn a working validator into `no_verdict`.
- **It is one deadline for the adapter, not one per call.** Four sequential 5s budgets could spend 20s while `.supertool.json` gave the whole adapter 5s, so on any real stall the core's budget fired first and killed the adapter mid-git — it never got to say anything. The registered `timeout` is now `30`, and it has to stay above the adapter's own budget plus its grace period, or the decline is unreachable and a tie is a race.
- **It costs interactive latency, and that is the trade rather than a free win.** Every mutating op blocks on its slowest validator — `_validators_run_batch` runs them concurrently with each other, but the `edit`/`replace`/`paste`/`vim` call waits for the batch. So on a repository whose git genuinely does not answer, an edit now blocks for **19s measured** (15s budget, then the SIGTERM grace, then process teardown) before the decline is printed, bounded above by the registered `timeout` of 30. It was about 5s before, because the core killed the adapter at its own budget — **5s and a wrong answer**. Roughly 3.4x worse, paid on every edit to a file in such a repository rather than only on a genuine hang, and it is the direct cost of not lying. **The knob cuts both ways and that is the remedy**: a stall is now an honest `NOT CHECKED` rather than a fabricated `clean`, so `SUPERTOOL_GIT_TIMEOUT=3` is a legitimate setting for somebody who would rather have a fast decline than a slow measurement. That was not a choice anybody could make before, because the fast answer was a lie.
- **A stalled git gets `SIGTERM` before `SIGKILL`.** `subprocess.run(timeout=)` calls `Popen.kill()`, which is SIGKILL on POSIX with no grace, so git never ran its own cleanup and the `.git/index.lock` it held was stranded — failing every later `git add`, `commit` and `checkout` in that repository until somebody deleted the file by hand. The reporter lost hours to exactly that. **The adapter still never deletes a lock itself**, and that is a decision rather than an omission: it cannot tell a lock it orphaned from one a concurrent `git commit` legitimately holds, and deleting the second corrupts that commit. Letting git clean up after itself fixes the damage without the inference. (Windows gets the grace period too and gains nothing from it — `terminate()` and `kill()` are both `TerminateProcess` there — which is a property of the platform, not a reason to branch on it.) **Superseded by #1944 for this adapter's own calls**: `run()` now passes `--no-optional-locks` (a git global flag, so it precedes the subcommand) on every call, and git skips the index writeback entirely for a read-only invocation carrying it — none of the four calls this adapter makes ever takes `.git/index.lock` at all any more, so the SIGTERM-then-grace mechanism above has nothing left to unlink for them. It still runs on every stall, because reaping the child is worth doing regardless and it is the fallback for any call that does not carry the flag.
...
The second is the interesting one, because the field looked like it had no room for a third state. It does: the suffix is a space-separated token list whose members already run to `non-utf8`, `crlf` and `->target broken`, so the one-character appearance came from its busiest members, not from a constraint. The decline is `git?` — it names the check that declined and cannot be read as any of the three answers it replaces, none of which mention git. A free punctuation mark was the alternative and was rejected on the grounds that made it free: a character nobody has a meaning for yet is the wrong carrier for the token that has to say the most. **Absence still means clean, and now means only that.** A machine with no `git` at all stays silent by the same reading — nothing there was ever going to answer, so a decline that can never resolve would be noise on every read. A file outside any repository used to be silent the same way (`not a git repository` is an answer, and the common one) but is not any more: [#1397](https://github.com/Digital-Process-Tools/claude-supertool/issues/1397) found the per-path `git status` spawn that produced that silence was itself wasted work — the repo-root walk `_path_meta_suffix` already runs had already established there is no repository, so the spawn could only repeat that answer or fail. That case now skips the spawn and renders a fourth, distinct token, `no-git` — "the walk found no repository; git was never asked" — because the walk has known gaps (an unreadable `.git`, a `GIT_DIR` that escaped the #714 scrub) where silently trusting it would be this exact defect class again: an absence produced by the tool, read as an absence in the world.
...
**A timeout used to say only that it waited — the four warm MCP adapters now say what they received** ([#1927](https://github.com/Digital-Process-Tools/claude-supertool/issues/1927), follow-up to [#1924](https://github.com/Digital-Process-Tools/claude-supertool/issues/1924)). `phpunit-mcp`, `phpstan-mcp`, `phpmd-mcp` and `rector-mcp` all speak JSON-RPC over a Unix socket to a warm daemon, through `validators/common/ndjson_scan.receive_until`. On the incident that opened the issue, the daemon's own answer had left the server 2 seconds into a 300s call — a forked test process's fatal leaked an HTML error page into the stream — and the client waited out the remaining five minutes with nothing in the receipt to say so. Three changes:

- **Two deadlines, not one.** `call_timeout` still bounds the whole exchange, but `idle_timeout` (10s by default) is shorter and measured from the *last* byte received, once any byte has arrived — a server that stops mid-exchange does not resume, so nothing is gained by waiting out the rest of the call budget. Total silence is unaffected: with zero bytes ever received there is no last byte to measure idleness from, so that case still runs the full `call_timeout`.
- **Silence and garbage are different failures, and the message says which.** `ndjson_scan.describe_timeout` reports `no bytes received` when nothing arrived, or the byte count plus the first ~200 bytes of what did, when it did — the discriminator #1924's `describe_buffer` already made, extended to quote the content rather than only count it.
- **The daemon's own log rides along.** `read_daemon_log_tail` reads the last few lines of the daemon's `<sock>.log` — written already, by `presets/mcp/daemon.py`, as the exchange happens — and folds them into the timeout message when the file is there to read. Best-effort: an unreadable or absent log is silent, not a second failure layered onto the first.

The elapsed time is always in the message (`no id=... response after N.Ns (...)`), so `NOT CHECKED` no longer reads the same at 2s and at 300s.

**A `NOT CHECKED` used to be the end of it even when a retry could have recovered — now the pipe gets one, and only where there is a reason to believe it will help** ([#2449](https://github.com/Digital-Process-Tools/claude-supertool/issues/2449)). The daemon behind all four adapters serves one client's request/response exchange at a time; if a client disconnects mid-exchange, bytes already in flight for it can still be sitting in the socket buffer when the next client is handed the same process. That next client's `receive_until` then finds a well-formed JSON-RPC response addressed to somebody else's id, never finds its own, and — before this — raised the same plain `RuntimeError` a genuinely dead or genuinely slow daemon raises, indistinguishable from either. The write that triggered the check had already been accepted regardless; nothing escalated, retried, or respawned.

`_scan` (the function behind both `find_response` and `receive_until`) now tells two things apart: a response addressed to a *foreign* id — one that is neither the id this call is waiting for nor `INITIALIZE_ID` (every exchange's own `initialize` frame gets a response with that id first, and that alone is not evidence of anything wrong) — is real evidence of a desynchronised pipe and raises `DesyncDetected`, a `RuntimeError` subclass. Plain silence, or noise that never parses as a JSON-RPC response at all, still raises the original `RuntimeError`, unretried: it is exactly as consistent with a daemon (or an analysis) that is simply slow or genuinely gone, and respawning on that signal would turn every such call into a silent double-timeout for no corresponding chance of recovery.

Each adapter's `ndjson_call` retries **exactly once**, against a daemon force-respawned by the new `presets/mcp/_spawn.force_respawn` (which reaps the current daemon under the same spawn lock `ensure_daemon` uses, even though its fingerprint still matches — the ordinary discovery path has no way to see a desync, since the daemon is alive and its config is current), through the shared `ndjson_scan.call_with_retry`. A second `DesyncDetected`, or any other failure out of the retried attempt, propagates unchanged: the fix closes the *class* the issue asked for — never silently reporting `NOT CHECKED` after a write was accepted without at least once trying to recover — without turning a genuinely dead daemon into two full `CALL_TIMEOUT_SEC` waits for the same answer.

**Three gaps in that first pass, all found in review rather than shipped.** The daemon is one shared process per `(cwd, name)` serving every caller's exchange serially, and that sharing is exactly what the fix above did not fully account for:

- **`force_respawn` can kill a healthy, unrelated in-flight exchange.** It reaps whatever is currently answering unconditionally once it holds the lock, with no way to tell a genuinely healthy caller mid-exchange from a desynchronised one — so a *different* caller's connection can be severed collaterally by *this* caller's respawn, and that collateral caller sees a plain `RuntimeError` (a broken pipe, not a foreign id), which the original fix did not retry. `ndjson_scan.call_with_retry` now takes an optional `pid_probe` -- a zero-argument callable returning the daemon's current pid -- read once before the call and again after a plain `RuntimeError`. A pid that changed between the two reads means something else replaced the daemon mid-exchange, almost certainly this exact collateral case, and earns the same single retry a self-detected desync gets, without a second `force_respawn` (a fresh, usable daemon already exists at the same socket path). A pid that did not change is the ordinary case this function has always declined to retry.

  **The probe must name the pidfile, never resolve it.** Each adapter's `pid_probe` derives it from the socket path already in hand, via `presets/mcp/_spawn.pid_path` -- the same sibling-stem derivation `lock_path` and `fingerprint_path` have always used. It must not reach for `_paths.socket_pid_paths(cwd, name)`, which computes the identical string but gets there through `runtime_dir()`, and `runtime_dir()` creates the directory, validates it, and `sys.exit`s wherever ownership cannot be checked (#544 -- anywhere `os.geteuid` is absent). `_paths` states the invariant that keeps that refusal off the warm adapters: they decline for want of `AF_UNIX` first, so no warm validator ever reaches it. The first cut of `pid_probe` did reach it, and turned `ndjson_call` into a `SystemExit` on Windows -- 56 failures on each of the four `pytest (windows-latest, *)` legs, every other leg green, including tests that predate #2449 entirely. A real Windows *run* never got that far (`ensure_daemon` declines for want of `AF_UNIX` before there is a socket path to call `ndjson_call` with, which is also why the #544 tripwire could not see this -- it asserts about the pre-daemon path, where the decline still works), but the resolution was worth removing everywhere regardless: it ran on **every** call, since `call_with_retry` probes the pid before the first attempt, and `runtime_dir()` creates the directory, opens it and validates ownership and mode rather than merely looking a path up. Naming a sibling of a path already held asks the filesystem nothing, which is what a probe reading a pidfile that may not exist should do.
- **The `initialize` frame's id was still the shared literal `1` on every call**, so a foreign client's *own* leftover `initialize` reply (also `id: 1`, if that client disconnected between sending its `initialize` and reading the reply) was indistinguishable from this exchange's own — masking exactly that one interleaving of the desync #2449 exists to catch. Each adapter now draws its `initialize` id at random too (same treatment #1935 already gave the `tools/call` id), and passes it to `receive_until` as `own_ids` rather than relying on the old shared default (`ndjson_scan.INITIALIZE_ID`, kept only as the default for a caller that does not supply its own). `req_id` is drawn first and `init_id` is nudged by one on the (otherwise negligible, but deterministic under a test that pins `random.randrange` to one fixed value) chance the two collide, so a caller pinning that mock for its own reasons still gets the *call* id it expects.
- **The retry's own worst case can exceed the shipped example config's timeout.** `CALL_TIMEOUT_SEC` (twice, one per attempt) plus the respawn's own lock-wait and spawn-wait sits well above `.supertool.example.json`'s pre-#2449 `timeout` for all four validators — phpstan-mcp's worst case grew to roughly 484s against a configured 300s, for instance — so the core's own budget could cut the retry off before it ever paid for itself, on the maintainers' own recommended config. The four `timeout` values in `.supertool.example.json` (and the two adapter `README.md`s that publish their own sample) are bumped with headroom over the new worst case: 540 (phpstan-mcp), 400 (phpmd-mcp), 780 (phpunit-mcp), 400 (rector-mcp).
...
**The same non-verdict reaches the tests asserting the opposite** ([#1296](https://github.com/Digital-Process-Tools/claude-supertool/issues/1296)). #794 covered the direction where a test asserts a file is **clean**, and stopped there because that is where it was observed. `tests/test_html_check.py` asserts the other one: broken inline JS is a finding, pinned to the source line node reported it on. A stall is `ok: false` with one `adapter` error, so `assert_declined` **passes** and the pinned line — which the adapter never reached — is what fails. Same non-verdict, opposite assertion, and a wrapper built around `assert_ok` could not be reused for it. `stalled_at_its_own_wall()` was not touched by *that* change — the paragraph below is where it moves — and now also backs `skip_if_stalled(payload, inner_s=…)`, which declines a stall and returns anything else untouched; `test_html_check.py::_run` routes all 32 of its spawn sites through it once, rather than each assertion guarding itself. Observed twice on `pytest (windows-latest, 3.11)` on 2026-08-11, both cleared on a re-run; why that leg and not the others on the same job is unexplained and nothing here claims otherwise.
...
**A third state sits underneath the refuse/warn split, for the branch where the caller already opted out** ([#2493](https://github.com/Digital-Process-Tools/claude-supertool/issues/2493)). `literal_backslashes` (#1096, above the same section) suppresses the doubled-backslash refusal per field, not per occurrence — so one legitimate doubled run (a regex, a Windows path) in a `content`/`new` field also waives every OTHER doubled run in that field, including a genuine `\n`-typo doubled by reflex. `py-syntax` cannot see this at all: `"a\\nb"` and `"a\nb"` are both syntactically valid Python with different runtime values, and it only ever answers "does this parse" — the same limit `_PY_SYNTAX_SCOPE` states above. Rather than either widen the refusal (which would re-strand the caller `literal_backslashes` exists to unstrand) or say nothing (the exact absence this section is about), a `.py`-only advisory rides the same write chokepoint `_WRITE_WARNINGS` already uses (see `_sh_backslash_warning`, a few sections up): it names the field and the byte offset of every exempted occurrence, appended to the RECEIPT after the `[validators]` block — never to `py-syntax`'s own verdict, which stays `ok` because it is correctly answering a different question, and never blocking or rolling back a write that has, by construction, already happened.
