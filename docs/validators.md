...
- **`N` is read only from the first framed line**, the same discipline `_framing` uses: the real header precedes every diagnostic, so it is the one position the file under validation cannot get in front of the server. A forged `N` is a forged verdict.
