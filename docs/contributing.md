...
**The first four columns belong to the assembler**, and this is the one rule about fragment *content*. Because nothing is reformatted, a fragment line that CommonMark would read as a heading or a link-reference definition becomes one in `CHANGELOG.md` itself: it reparents every entry below it, it is what the assembler's insertion point finds on the *next* cut, and a definition lands above the genuine link-ref block at the bottom, where the *first* definition of a label is the one that resolves ([#923](https://github.com/Digital-Process-Tools/claude-supertool/issues/923)). `--check` refuses such a fragment naming the file, the line number and the line, so it fails on the PR rather than in front of whoever is cutting the release.

**Column 0 was the wrong boundary** ([#930](https://github.com/Digital-Process-Tools/claude-supertool/issues/930)). CommonMark allows **0-3 leading spaces** before both constructs, link labels are **case-insensitive**, and *any* `[label]:` line is a definition — so a heading indented by one space, a lowercase `[unreleased]:`, and a `[docs]:` line each walked through a guard that then reported `ok` while the released file carried them. The rule is now 0-3 spaces, any case, any label, and the `--check` receipt states the property rather than the anchor it used to be implemented with.

**Indent by four spaces to quote a heading in prose.** Entries here quote headings constantly — [#839](https://github.com/Digital-Process-Tools/claude-supertool/issues/839) is one — and the bullet-plus-indented-paragraphs format already indents everything after the first line, so the remedy is the format. Four, not the two the old refusal message prescribed: at four columns CommonMark reads an indented code block and every scanner in the assembler reads something inert, while at one to three it reads a heading. A leading tab is not refused, because a tab advances to the next four-column tab stop and is therefore already at or past that boundary — widening the pattern speculatively costs an author a safe line, and a guard that cries wolf gets worked around rather than obeyed. The rule is deliberately **not** fence-aware: a fenced code block does not make an unindented heading safe, because none of the assembler's own scanners, and none of git's merge machinery, know what a fence is. A half-aware fence parser that mishandled nested or tilde fences would be worse than a blunt rule that is written down.
