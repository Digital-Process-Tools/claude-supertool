...
**`{arg}` is deliberately not a signal**, even though it substitutes the same `parts[1]` that `{file}` does. 20 shipped ops carry `{arg}` (24 until [#873](https://github.com/Digital-Process-Tools/claude-supertool/issues/873) moved three multi-token ops to `{args}`, 21 until [#1715](https://github.com/Digital-Process-Tools/claude-supertool/issues/1715) moved `gh-run`); 8 of them name a path in `syntax` and are already held by the first detector, leaving 12 that pass a handle, a ref, a tag, an ID or a repo slug and take no path. Promoting `{arg}` would refuse those 12 and gate nothing. If your op means a path, write `{file}`.
