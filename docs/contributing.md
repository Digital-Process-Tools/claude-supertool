...
**A pre-subcommand option the matcher has no grammar for is `undecided`, never normalised on a guess** ([#1421](https://github.com/Digital-Process-Tools/claude-supertool/issues/1421)). `_GUARD_GLOBAL_OPTIONS` in `_supertool.py` carries, per command word, which options take a value and which do not; anything in neither list makes the verdict `undecided` and the command runs with the reason attached. That is deliberately the smaller claim. Skipping leading dashes until a positional turns up walks past the subcommand itself on a malformed command, and guessing an option's arity swallows the subcommand into a value — both produce a **wrong block**, whose only escape is repo-wide. `git --exec-path` sits in neither list for exactly this reason: bare it prints a path and runs nothing, with `=` it takes a value, and one token with two arities is what the third state is for.
