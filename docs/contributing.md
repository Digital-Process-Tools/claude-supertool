...
- **Do not declare a command whose `-h` is a real option.** The guard treats `-h` and `--help` as terminal for every entry in every registry — it has no per-command arity and will not grow one — so a mapping for `ls`, `du`, `sort` or `grep` gets a `clean` verdict on `du -h`, `ls -h`, `sort -h` and `grep -h`, where the token is a real option and the command runs ([#1452](https://github.com/Digital-Process-Tools/claude-supertool/issues/1452)). `clean` is a positive claim that nothing here is replaced, so this is the round-1 defect shape reintroduced by configuration rather than by code. Nothing shipped is reachable this way: all 28 entries are `git` or `gh`, and every argv the classifier calls `help` was executed against git 2.46.2 — all exit 129, commit count unchanged, no push attempted. If you need such an entry, say so on the tracker: the fix is a per-entry declaration that this command's `-h` is not help, and it is unbuilt because nobody has needed it.
