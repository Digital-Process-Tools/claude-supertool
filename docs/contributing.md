...
| Python, **only the files a PR adds, copies or renames** | the `lint-new` job, ubuntu + py3.12 | `.github/scripts/lint_new_files.py` — `ruff check` over those paths alone, with `F401`/`F841`/`F541` re-enabled for them |
