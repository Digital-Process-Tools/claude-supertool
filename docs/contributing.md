...
**Column 0 belongs to the assembler**, and this is the one rule about fragment *content*. Because nothing is reformatted, a fragment line starting at column 0 with `#`, `[Unreleased]:` or `[1.2.3]:` becomes a heading or a link-reference definition of `CHANGELOG.md` itself: it reparents every entry below it, it is what the assembler's insertion point finds on the *next* cut, and an `[Unreleased]:` line lands above the genuine link refs where the release rewrite would have found it first ([#923](https://github.com/Digital-Process-Tools/claude-supertool/issues/923)). `--check` refuses such a fragment naming the file, the line number and the line, so it fails on the PR rather than in front of whoever is cutting the release.

**Indent to quote a heading in prose.** Entries here quote headings constantly — [#839](https://github.com/Digital-Process-Tools/claude-supertool/issues/839) is one — and the bullet-plus-indented-paragraphs format already indents everything after the first line, so the remedy is the format. An indented heading is inert to every reader of the file; a column-0 one is not. The rule is deliberately **not** fence-aware: a fenced code block does not make an unindented heading safe, because none of the assembler's own scanners, and none of git's merge machinery, know what a fence is. A half-aware fence parser that mishandled nested or tilde fences would be worse than a blunt rule that is written down.
