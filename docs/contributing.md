...
The assembled descriptive render, `ops:full`, is ~89.76KB (moved from ~89.55KB by #2665's `gh-issue-comment` GET-before-PATCH ownership-check description, itself moved from ~89.11KB by #2643's `gh-issue-comment` `edit=COMMENT_ID` description, itself moved from ~88.71KB by #2592's git-commit control-byte refusal, itself moved from ~87.46KB by #1985's `oss-tick` op description, itself moved from ~87.11KB by #1315's `gl-pipeline`/`mcp_stop` description and syntax text, and from ~86.89KB by #2536/#2544's desktop-notification knob text) against `_HOOK_OUTPUT_CAP_BYTES` = 10,000. That is why `hooks/session-start.sh` sends `ops:roster` instead: names plus safety class, no descriptions at all. Bare `ops` has been signatures-only since #1774/#1775/#1778 and fits the cap on its own at ~4.70KB (moved from ~4.58KB by the same op) without needing a fallback (issue [#1813](https://github.com/Digital-Process-Tools/claude-supertool/issues/1813) is where the earlier version of this paragraph is corrected). It is the *descriptions* — reachable only through `ops:full` and `help:OP` now — that have never fitted, and this section is about what they cost.
...
| `unless_args` | no | How many positional arguments past `argv` the op can express. More than this un-claims the entry and is **disclosed** as `uncovered` ([#1684](https://github.com/Digital-Process-Tools/claude-supertool/issues/1684)). A non-negative integer. |
| `use` | no | The op invocation the refusal names. Defaults to the op's `syntax`. |

**The refusal names the op's `@payload` route on its own line, above the description** ([#1815](https://github.com/Digital-Process-Tools/claude-supertool/issues/1815)). The description is quoted from the registry and cut at `_GUARD_DESC_CAP` — by position, because there is no relevance to cut by — so a route documented 3.5KB into a 4KB `description` does not reach the caller. That is where `git-commit`'s was: `git commit -F -` is the one raw spelling that takes a multi-line message, and it was refused, pointed at `git-commit`, and shown a contract whose visible half did not contain `git-commit:@-`. The line is rendered from `_at_file_specs`, the same registry the route itself uses, so it cannot advertise a route a `syntax` reword has deleted; an op with no `:::` in its syntax prints no such line, and that emptiness is an answer rather than a gap. **You write nothing to get it** — the route follows from `syntax`, which is the same fact the `syntax` bullet above already warns you not to reword casually.
...
- **It un-claims the entry, it does not allow the command.** An exclusion loses to nothing: a second, broader entry that still matches still blocks. A veto that crossed entries would let an op in a repository's own `.supertool.json` un-block a command a shipped op legitimately claims. The same is true of `unless_args`, with one difference: an arity decline is **disclosed** rather than silent, because "this op takes no ref" is a fact the caller can act on, while "this command carries `--dry-run`" is one they already know.
...
- **A help flag needs no `unless_flag` and cannot be overridden by one — but only where the program would read it as a help flag.** `--help` and `-h` un-claim every entry in every registry, because a command that only describes itself performs nothing an op could supersede. Measured on the v0.35.0 tree, all 28 shipped mappings blocked their own `--help`, and each refusal named the op that does the thing the flag declines. Whole tokens only — unlike `unless_flag`, a short cluster is **not** expanded, or `-xh` would un-claim the whole repository at once.

  **A help token immediately after another flag may be that option's value, and is blocked rather than honoured.** `git commit -m -h` commits with the message `-h`; `git push origin -o -h` pushes. Both were `clean` until the v0.36.0 round-1 audit, because the scan ran over the whole option slice and asked only whether a token equalled `-h`. Restricting it to flag-shaped tokens is a no-op — `-h` is flag-shaped in a value slot too. `_guard_help_state()` answers `help`, `value` or `none`, and the position is what separates them: measured on git 2.46.2 and gh 2.50.0, `git push origin -h`, `git push -h`, `git status -h` and `gh issue list -h` all print usage and run nothing, so a help token after a *positional* still un-claims. Only the slot after another flag is in doubt, and telling a value from a request there needs per-subcommand option arity this guard does not carry. It blocks, with a note: a wrong block on `git push origin -o -h` is legible and one flag away from a working `git push -h`, and the other direction is a silent `git push`. (This paragraph cited `git push --force -h` until [#1452](https://github.com/Digital-Process-Tools/claude-supertool/issues/1452) — `--force` is an `unless_flag` of every `git push` entry, so that command is un-claimed before the help classifier runs and never blocked. `tests/test_guard_help_example_and_project_entries_1452.py` now runs every command these docs call a wrong block against the guard.)

  **The same three states apply to an `unless_flag` token in a value slot, and the call goes the other way** ([#1450](https://github.com/Digital-Process-Tools/claude-supertool/issues/1450)). `gh pr create -t --dry-run -b y` creates a pull request **titled** `--dry-run`, and reading that `--dry-run` as an exclusion made the guard answer `clean` about a command that opens a PR. It is not blocked, because the flags entries exclude are the ones people genuinely combine: seven shipped invocations flip clean → blocked if the slot is scored, five of them previews of a destructive push (`git push --force-with-lease --dry-run`), and a refusal that names `git-push` under a blocked `--dry-run` performs what the command declined to do. So the entry stays un-claimed, nothing new is blocked, and the segment is `undecided` with a note instead of `clean`. A help flag is terminal and rarely combined, which is why the same ambiguity is blocked there and disclosed here.

  **The window is narrower than "after any flag" for git** ([#1816](https://github.com/Digital-Process-Tools/claude-supertool/issues/1816)). The guard reported this gap on `git commit --amend` behind another flag — the invocation it would least like to be unable to read — so `_GUARD_VALUELESS_FLAGS` carries which flags consume no separate value for the four git subcommands the shipped presets gate, measured from `git <sub> -h` on git 2.46.2 rather than reasoned. `--no-verify` eats nothing, so `git commit --no-verify --amend` is un-claimed for a stated reason and stays silent. **The same table decides whether a help flag behind another flag is a request for help** ([#1832](https://github.com/Digital-Process-Tools/claude-supertool/issues/1832)). `git commit -a -h` prints usage and commits nothing (exit 129), and the guard used to score the `-h` as `-a`'s value and refuse the command toward `git-commit` — an op that commits, so the refusal named a next step doing something the caller never asked for. `-a` is in the row, so `-h` behind it is now read as help and nothing is blocked; `git commit -m -h` is unmoved, because `-m` is not. That clause is the one edge in this family that turns a block into an **allow**, so it is only ever as good as the table: every spelling the four rows generate — 101 of them — was run against git 2.46.2 and every one printed usage, and `tests/test_guard_help_behind_valueless_flag_1832.py` re-derives that set from the table so a value-taking flag added to a row later fails there rather than shipping as a wrong allow. A cluster is answered for the whole cluster: `-as` is valueless and un-blocks, `-am` ends in `-m` and does not. **Only the valueless half is tabled**, and a flag absent from it stays where #1450 left it: ambiguous, allowed, disclosed. Both ways the table can go stale land on allow-more — a flag git adds is absent and unchanged, a flag git gives a value to is read as standing and un-claims the entry — and neither can manufacture a block. gh is deliberately not in it: `_GUARD_GLOBAL_OPTIONS` already records that gh moves flags between its root and its leaves, so `gh pr create --draft --dry-run` stays disclosed. The note now also names **which op's claim went unevaluated** and what a readable re-issue would be, because "a check was skipped" and "whether `gh-pr-create` replaces this command was never asked" are different sentences to act on.
...
- **Declare only what the op genuinely replaces.** Absence *is* the escape hatch. There is no op for tagging, releasing, deleting a ref or re-running a workflow, so nothing maps `git tag`, `gh release create`, `gh api -X DELETE` or `gh run rerun` and they are never blocked. An entry for something the op only half-answers turns a working command into a dead end — `git push origin <tagname>` was one for two releases, and `unless_args` is how it stopped being one.
...
- **Do not declare a command whose `-h` is a real option.** The guard treats `-h` and `--help` as terminal for every entry in every registry — it has no per-command arity and will not grow one — so a mapping for `ls`, `du`, `sort` or `grep` gets a `clean` verdict on `du -h`, `ls -h`, `sort -h` and `grep -h`, where the token is a real option and the command runs ([#1452](https://github.com/Digital-Process-Tools/claude-supertool/issues/1452)). `clean` is a positive claim that nothing here is replaced, so this is the round-1 defect shape reintroduced by configuration rather than by code. Nothing shipped is reachable this way: every shipped entry is a `git`, `gh` or `glab` invocation and all three print usage for `-h`, which `tests/test_guard_help_example_and_project_entries_1452.py` pins as a property rather than as a count. (This paragraph cited "28 entries" until #1452: that was a v0.35.0 measurement, and `git` and `gh` alone are 29 today.) Every argv the classifier calls `help` was executed against git 2.46.2: all exit 129, commit count unchanged, no push attempted. If you need such an entry, say so on the tracker: the fix is a per-entry declaration that this command's `-h` is not help, and it is unbuilt because nobody has needed it.
...
- **The value of a positional.** `git push origin master` and `git push origin v0.34.0` are the same argv shape, the same arity and the same token classes, and no entry can tell them apart — that would mean asking the repository whether a ref is a tag, at guard time, on every Bash call, before the command runs.

  **It is the wrong question, and asking it cost two releases** ([#1684](https://github.com/Digital-Process-Tools/claude-supertool/issues/1684)). This paragraph used to conclude that `presets/git.json` therefore claims both, and that pushing a tag by name is blocked with `git-push` named — "a **wrong block, disclosed** rather than avoided". It was worse than a wrong block: `git-push` pushes the current branch, so a caller obeying that refusal published a ref they never named while the tag stayed uncreated, and the command reported success. **Both** commands name an explicit refspec and `git-push` names none, so `unless_args: 1` declines both without knowing what either ref *is*, and the caller is told that no op covers this form rather than handed one that does something else. **The disclosure then went on to hand them one anyway** — it closed "`git-push` is the same invocation without them, if that is what you meant" until [#1707](https://github.com/Digital-Process-Tools/claude-supertool/issues/1707). Same defect one sentence later, and the rule it leaves for anyone writing a `replaces` entry is: a note may say what an op *performs*, never that dropping the tokens the op does not take leaves the caller's own command. What a positional's value still cannot do is select *which* op is named; that is what `flag` and `value` are for.
- **A global option before the subcommand — fixed in the matcher, not in `replaces`** ([#1421](https://github.com/Digital-Process-Tools/claude-supertool/issues/1421)). `argv` matches a contiguous token prefix, so `{"argv": "git status"}` did not match `git -C /tmp/x status`, `git -c core.pager=cat status` or `git --git-dir=/tmp/x/.git status`, and each rendered `OK: nothing in this command is replaced` — byte-identical to a command that genuinely has no replacement. The spelling `replaces` offers, `{"argv": "git -C"}`, claims *every* `git -C` subcommand including `git -C W tag v1`, so the entry stays as written and `_guard_normalise` strips the options before scoring instead. Nothing to write in a mapping; what you need to know is the third state below.

**A pre-subcommand option the matcher has no grammar for is `undecided`, never normalised on a guess** ([#1421](https://github.com/Digital-Process-Tools/claude-supertool/issues/1421)). `_GUARD_GLOBAL_OPTIONS` in `_supertool.py` carries three lists per command word — which options take a value, which are pass-through booleans, and which are **terminal**; anything in none of them makes the verdict `undecided` and the command runs with the reason attached. That is deliberately the smaller claim. Skipping leading dashes until a positional turns up walks past the subcommand itself on a malformed command, and guessing an option's arity swallows the subcommand into a value — both produce a **wrong block**, whose only escape is repo-wide. A **terminal** option ends the command, so the subcommand written behind it is never dispatched and the verdict is `clean` rather than `undecided` — `git --version status` prints the version and never runs `status`, and treating it as an ordinary boolean scored `git status` and blocked it ([#1437](https://github.com/Digital-Process-Tools/claude-supertool/issues/1437)). Terminal matching is on the exact token: bare `git --exec-path` prints a path and runs nothing, while `--exec-path=P` takes a value and does run the subcommand, so the `=` spelling is in no list and stays `undecided`.

Two things that table is not. It is **not git-only**: `glab` puts `--repo` / `-R` on its root command and is normalised the same way. And it carries **no `gh --repo`**, because `gh` puts that flag on its leaf commands — `gh --repo cli/cli version` answers `unknown flag: --repo` (observed, gh 2.x) — so an entry for it would block a call that cannot execute. `gh --repo O/R pr view 1` is `undecided`.
...
**Three states, and the third is the reason this exists at all.** A rule that is indexed, named in `SHIPPED` and impossible to honour — an untranslatable pattern, a body missing from disk, an index that is not UTF-8 — is a `note` on every matching call, not silence: silence there is a rule that looks shipped and is not, which is the shape [#1698](https://github.com/Digital-Process-Tools/claude-supertool/issues/1698) was filed about, reproduced inside its own fix. It is bounded to an install whose index is *present*: a plugin root with no rule directory carries no layer rather than a broken one, and a note there would ride on every Bash call anyone ever makes.
...
The core's opt-outs (`SUPERTOOL_ALLOW_OUTSIDE_CWD=1`, or `"allow_outside_cwd": true` in `.supertool.json`) are honoured at every boundary, and the refusal names them. The gate is `_safe_path` with a different root, not a second copy of the rule — [#882](https://github.com/Digital-Process-Tools/claude-supertool/issues/882) and [#889](https://github.com/Digital-Process-Tools/claude-supertool/issues/889) are what a second copy costs. The refusal also names `cwd:PATH` first, ahead of either opt-out ([#1784](https://github.com/Digital-Process-Tools/claude-supertool/issues/1784)): moving the boundary for one call leaves no residue in either repository's config, which is what a one-off read of a file in another root usually wants.
...
**A template with no argument placeholder at all is held to the same rule** ([#1532](https://github.com/Digital-Process-Tools/claude-supertool/issues/1532)). `"cmd": "make lint"` called as `op:x` used to drop `x` exactly as silently as the single-placeholder case above — nothing in `parts[1:]` can ever reach a template that names no placeholder, so every such token is refused too, including the path `check:PRESET:PATH` always forwards to whatever entry was named.
...
Any key in an op config that isn't a reserved key (`cmd`, `timeout`, `description`, `syntax`, `example`, `status`, `restartMcp`, `replaces`, `paths`, `exitStatus`, `form`, `hint`, `safety`, `repo_target`) is passed to the subprocess as a `SUPERTOOL_`-prefixed environment variable. `form` and `hint` are documented as `builtin-ops`-only keys ([#1245](https://github.com/Digital-Process-Tools/claude-supertool/issues/1245)) — a `builtin-ops` entry never spawns, so they were only inert oversights there. Reserved here anyway ([#1675](https://github.com/Digital-Process-Tools/claude-supertool/issues/1675)), because the moment either is copied onto an **`ops`** entry it would otherwise leak as `SUPERTOOL_FORM` / `SUPERTOOL_HINT` — config metadata reaching a subprocess environment as if it were meant for the script. `safety` and `repo_target` join the list for the same reason ([#1757](https://github.com/Digital-Process-Tools/claude-supertool/issues/1757)): both are read only by the core off the merged registry entry — safety classification, whether a leading `repo:OWNER/NAME` token is honoured — never by the op's own subprocess, so exporting them as `SUPERTOOL_SAFETY` / `SUPERTOOL_REPO_TARGET` would be a fact about the op leaking into the process it describes:
...
**The directory is currently empty and the convention is kept anyway.** `oss_train` was the only op that ever lived here and [#1472](https://github.com/Digital-Process-Tools/claude-supertool/issues/1472) deleted it: `changelog.d/` fragments removed the merge-conflict churn it automated ([#906](https://github.com/Digital-Process-Tools/claude-supertool/issues/906)), `git-push` rebases the one branch that needs it at the moment it needs it, and three containment-class repairs in four days ([#1246](https://github.com/Digital-Process-Tools/claude-supertool/issues/1246), [#1247](https://github.com/Digital-Process-Tools/claude-supertool/issues/1247), [#1298](https://github.com/Digital-Process-Tools/claude-supertool/issues/1298)) landed on a tool with no caller. Two things survive it, and both are checks rather than prose: `tests/test_git_status_display_config_gates_1295.py` still sweeps `scripts/` for `git status` gates and declares the directory's absence in `ABSENT_TREES` rather than reporting a clean zero, and the `{file}`-in-`cmd` half of the path-declaration detector — which `oss_train` was the only shipped instance of — is exercised by a fixture, described under **Path containment** above.

What is lost, stated rather than waved away: nothing else batch-rebases N branches in one call. That is deliberate. A batch force-push across directories that hold live agents is the most dangerous operation this repo ever shipped, and it should come back only with the contract those three issues cost — targets are names under a declared root, never paths; one `realpath` per target and the checked path is the used path; a dirty-tree guard that reads `git status` with the display config pinned.

**If you write one that reads `git-push`'s verdict, read it as a prefix.** `"PUSHED" in verdict` is true of `NOT PUSHED - REJECTED`, `- UNVERIFIED`, `- REBASE PAUSED`, `- TIMED OUT`, `- already up to date` and `- no push attempted` — every failure `git-push` emits. `oss_train` existed to relay a verified verdict rather than assume its own success, and ended on a membership test that tallied all six as `PUSHED`, with a detail line reading "NOT PUSHED - ..." next to the tick. The op is gone; the trap belongs to `git-push`'s output and outlives it.
...
**No parser means `skipped`, not `ok`.** `markdown-it-py` is a dev dependency and the assembler imports it. Without it the script has established nothing, so it says so, exits non-zero and writes nothing — there is no text-scanning fallback, because three of those shipped and all three were bypassed within one audit. `.github/workflows/oss-changelog.yml` installs it; the job used to be a checkout and a bare `python3`, which is worth knowing before assuming a CI leg has the dev extras.
...
**CI asks for a fragment, and says when it does not.** `.github/workflows/oss-changelog.yml` requires one from any PR touching a `user_visible_paths` pattern declared in `.oss.json` — `supertool.py`, `_supertool.py`, `presets/`, `validators/`, `formatters/`, `notifiers/` and `.claude-plugin/` here. Docs-only and tests-only PRs are exempt by design — requiring a fragment for a typo fix is how the discipline decays into a reflex-added empty file — and a `no-changelog` label is the stated way out for anything else. The job prints which state it landed in on every run.

**The label is read live, so applying it can actually clear the run that asked for it** ([#1722](https://github.com/Digital-Process-Tools/claude-supertool/issues/1722)). The check used to read `github.event.pull_request.labels`, and a run's event payload is fixed at creation: applying `no-changelog` started a fresh run that passed, nothing removed the run that had already failed, the merge gate is conjunctive over every run on the head sha, and re-running the failed one replayed the pre-label payload. So the remedy the job printed could not clear the job printing it, and the only route out was an empty commit — measured on #1721, twice. The job now reads `repos/{repo}/pulls/{pr}` at run time, which is why the workflow declares `pull-requests: read` — the `issues/{n}/labels` spelling the issue suggested is covered by `issues: read` instead, and a scope mismatch there would 403 on every run forever while looking installed. **Label the PR, then re-run the failed check.** The cost is stated rather than hidden: the verdict is no longer reproducible from the run's own payload, and a label *removed* after the run started reddens it — which it must, or an escape hatch outlives the decision to use it. If the live read fails — an outage, no `gh`, a token without the scope — the job falls back to the payload, **prints `note (live label read failed …)` rather than answering as though it had read something**, and switches its remedy line to "push a commit", because a re-run is exactly what that arm cannot honour. The fallback is the behaviour the job had before #1722, so it can only ever turn a finding into a skip and never the reverse: an API outage cannot redden the board.

Two details of that read are load-bearing and were both found by the independent review of the first cut. **The call is budgeted** (`timeout 30`, where `timeout(1)` exists — it is GNU coreutils, so ubuntu yes, stock macOS no): slow and down are different failures and only one of them returns an error to fall back on, and before #1722 this step made no network call at all, so an api.github.com that is degraded rather than down would otherwise sit here until the job's blunt `timeout-minutes: 5` cancelled it — red, receiptless, on every open PR at once. **And labels are matched one per line, never as a joined string.** A GitHub label name may contain a comma, so `,${LABELS},` against `*,no-changelog,*` could not tell the single label `wontfix,no-changelog` from the two labels `wontfix` and `no-changelog`; the live read has the labels separately and compares them whole. The payload fallback keeps the ambiguity because Actions joins with a comma before the shell ever sees it.
...
```bash
python3 .oss/assemble_changelog.py --version 0.26.0 --dry-run   # read it first
python3 .oss/assemble_changelog.py --version 0.26.0
```
...
```bash
python3 .oss/assemble_changelog.py --check-links --untagged '0.19.0,0.18.0,0.17.0,0.16.0,0.15.0,0.14.0,0.11.0'
```
...
**The scan declines rather than guessing.** `subprocess.run(cmd, **opts)` and `text=some_flag` cannot be judged syntactically, so they are reported as unreadable instead of being counted clean — see `test_the_subprocess_scan_declines_rather_than_guessing`. Spell the kwargs out literally at the call site. Known blind spots, stated so the green reads correctly: a call built through `getattr(subprocess, name)`, of which there are none today. An aliased runner (`_REAL_POPEN = subprocess.Popen`, `from subprocess import run as _r`) is resolved and judged, and `tests/` is held to the same rule as shipped code — both since [#862](https://github.com/Digital-Process-Tools/claude-supertool/issues/862). The earlier exclusion argued a test mis-decoding its own fixture output fails loudly on a runner rather than silently in a user's hands; [#856](https://github.com/Digital-Process-Tools/claude-supertool/issues/856) is the counter-example, where loud meant four Windows legs red on a `TypeError` about `None` that named nothing.

**3. A preset that prints non-ASCII must not depend on the console encoding.**

Presets run as separate processes and inherit none of supertool's stream setup. Supertool pins `PYTHONIOENCODING=utf-8` for every child it spawns, so an op invoked through supertool is covered without the script doing anything. The path that pin does not reach is a script a human runs straight from a shell — `python3 presets/github/starred.py 5` — and **every preset entry point whose own `print()` calls carry a non-ASCII literal must call `use_utf8_stdout()` as the first statement of `main()`** ([#1388](https://github.com/Digital-Process-Tools/claude-supertool/issues/1388)). `presets/git/*.py` is held to it unconditionally, glyphs or not, because conflict work is where the family started ([#308](https://github.com/Digital-Process-Tools/claude-supertool/issues/308), [#415](https://github.com/Digital-Process-Tools/claude-supertool/issues/415)). Import it from `presets/_console.py`; `presets/git/_git_common.py` still holds a byte-identical second copy for the git scripts, and folding the two is filed rather than done. The rule keys on the *called name*, so either import satisfies it.
...
**There is no exemption route and that is deliberate.** #1388 proposed a per-literal rule — a glyph must go through a degrading writer, or declare an exemption — and the per-literal framing does not survive contact: there are 325 non-ASCII print literals in shipped code, `_untrusted` exposes no writer for a glyph the *tool* wrote (`flat`/`fence`/`scrub` mark somebody else's text), and the property that decides whether a `print` survives is `sys.stdout.encoding`, which is settled once per process rather than per literal. Nothing an entry point could state would make a raising `print()` acceptable, and the precondition costs one line. Which codepage you are on decides how many files it is: cp1252 encodes `—`, `…`, `•` and `·`, so only `→`, `★` and `↳` raised there; cp437 and cp850 — the actual default of a US Windows console — encode none of the em dashes, so all 27 non-git offenders did.

One consequence, stated rather than left to be discovered: after `use_utf8_stdout()`, `_untrusted._stream()` reads `utf-8` and prints its guillemet markers rather than the ASCII fallback [#863](https://github.com/Digital-Process-Tools/claude-supertool/issues/863) added, which on a genuinely cp437 console is mojibake. That is already what the supertool path does, since `PYTHONIOENCODING=utf-8` reaches `_stream()` the same way; and the alternative here is not a degraded marker but a process that prints a correctly-degraded banner and dies four lines later.
...
- **There is still no `ruff check .` step, and `lint-new` is not one.** A tree-wide gate reds an unrelated contributor's PR the first time ruff ships a new rule in a selected category, and that cost lands on whoever pushes next rather than on whoever caused it — `.github/workflows/tests.yml` refuses it in those words and this has not changed. What [#1481](https://github.com/Digital-Process-Tools/claude-supertool/issues/1481) closed is the seam *between* that refusal and the post-edit `ruff` validator, which reports only what an edit newly introduces: a file that arrives whole introduces nothing relative to a baseline it does not have, so PR #1473 merged 22/22 green with three unreachable imports while every edit to the file said `ruff : ok (no new errors)`. The job checks a path a PR adds, copies or renames whole, because such a path has no history for "pre-existing" to mean anything about — and a path the PR merely modifies against that path's own merge-base revision, which is the bullet below.

  Two details that are not decoration. **`F401`, `F841` and `F541` are re-enabled for those files**, even though `pyproject.toml` ignores them tree-wide: that ignore was bought against 263 pre-existing findings in files that *have* a history, and a path with none has no share of it — without this the gate returns `All checks passed!` for the exact file the issue was filed about (measured at `ffe47ed`). And **renames are in scope**, because `presets/github/_release_gate.py` is `R065` rather than `A`, so a gate scoped to additions alone would have missed that instance. The cost of that, measured: 141 of 902 tracked `.py` files carry one of the three today, so renaming one of them reds until its author deletes the dead line.

- **A file the PR *modifies* is checked too, and against its own merge-base revision** ([#1849](https://github.com/Digital-Process-Tools/claude-supertool/issues/1849)). The `ACR` scope above left a hole neither gate could see: the tree-wide ignore hides an `F401` in a modified file locally, and `lint-new` was not looking at modified files at all. Measured on PR #1843 — the leg reported **one** finding, in the only file that PR adds, and the same rule set over the whole diff found **five**. The other four were orphaned `import pytest` lines in modified files, invisible to both gates simultaneously, and would have shipped. The shape that produces them is worth knowing: **deleting a fixture orphans whatever only it used**, and a refactor that removes code is exactly the diff that touches modified rather than added files.
