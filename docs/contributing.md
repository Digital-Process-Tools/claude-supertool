...
`oss_train` is the one that exists. It rebases every open branch onto `master`, resolves the conflicts, and force-pushes — 55 of the last 60 merged PRs touched `CHANGELOG.md`, so with N open PRs each merge re-conflicts the other N-1 ([#906](https://github.com/Digital-Process-Tools/claude-supertool/issues/906)). Four things about it are load-bearing:
