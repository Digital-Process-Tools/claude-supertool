...
**The disclosure has a channel on the affected host, and it covers both hooks** ([#1401](https://github.com/Digital-Process-Tools/claude-supertool/issues/1401)). `hooks/guard-selftest.py` needs no shell, and its `could not run` state is one fact about two dead hooks: it names `session-start.sh` alongside the guard, says the session gets no `./supertool` wrapper and no op roster, and gives the way back — `py -3 supertool.py 'ops:roster'`, and reading `./supertool` in the docs as that path. Reporting only the guard left the more visible loss with no account of itself, which is the same absence-read-as-absence one layer up.
