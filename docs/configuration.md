...
**`introduction` and `output-format` carry a shipped default** (#2342): with neither key set, both ops print a built-in default rather than a "No ... configured" non-answer — `introduction`'s default states mechanical, install-wide facts (path resolution, the `@-` payload route, TOML quoting), `output-format`'s demonstrates the envelope every op's output shares, measured against a real batched call. Same env-over-config-over-built-in convention as `git-commit`'s coauthor trailer (`presets/git/commit.py`'s `_DEFAULT_COAUTHOR`): an `SUPERTOOL_INTRODUCTION` / `SUPERTOOL_OUTPUT_FORMAT` env var wins over the config key, which wins over the shipped default, and setting either layer to `""`/`"none"`/`"off"`/`"false"`/`"no"`/`"0"` disables it — reaching the old "No ... configured" line, for a project that wants its session preamble bare.
...
```json
{
  ...
  "builtin-ops": {
    ...
    "grep": {
      ...
      "description": "Search (10 results def). CONTEXT=N lines around match. :no-auto-read suppresses single-file auto-read. :full (#1712) suppresses the per-line char cap for this call only.",
...
| `read` | `git_timeout_seconds` | 2 | Budget, in seconds, for the `git status` spawn behind the ` git?`/` m`/` ?`/` !` marker on every read (the repo-wide bulk snapshot, and its per-path fallback). A flat, tunable budget rather than one that scales with concurrency: parallel workers are separate `supertool` invocations with no shared coordination point, so raising a caller's own worker count is a reason to raise this too rather than something this key infers on its own ([#1398](https://github.com/Digital-Process-Tools/claude-supertool/issues/1398)). Env: `SUPERTOOL_READ_GIT_TIMEOUT_SECONDS` |
...
**The wrapper writes the envelope; the rung writes a verb and a reason** ([#1625](https://github.com/Digital-Process-Tools/claude-supertool/issues/1625)). `hooks/pre_bash_guard.py` prints `supertool-guard-v1 ` and one of `note`, `deny` or `clean` — then a newline, then the text if there is any. `hooks/pre-bash-guard.sh` matches that prefix to recognise an answer, and then builds the `hookSpecificOutput` document itself: every field name, `permissionDecision` and its only possible value `deny`, are literals in the shell script, and the rung's text reaches JSON only through the escaper [#1613](https://github.com/Digital-Process-Tools/claude-supertool/issues/1613) added — except for `clean`, whose `additionalContext` the wrapper always writes as empty, ignoring whatever text a rung appends.
...
That bounds the forgery rather than ending it. A rung that is already lying can still say `deny` with a misleading reason — though it could always deny by refusing to run at all, which prompts the same permission dialog a wrong deny would. What it can no longer write is `allow`, the one value that removes the user's own permission prompt, or any other field of the hook protocol. Reaching that sink at all still requires setting `$VIRTUAL_ENV` to a directory whose `bin/python3` the attacker controls, which is already an execution primitive; this is a fabricated verdict, not an escalation to code execution.

**`silent` was a third word, and it is gone** ([#1686](https://github.com/Digital-Process-Tools/claude-supertool/issues/1686)). It used to write the bare `{"hookSpecificOutput":{"hookEventName":"PreToolUse"}}` envelope — no decision, no note, nothing in the transcript — and that made it the one word a forger could claim for free: those bytes are exactly what the wrapper's own no-op writes, so a `$VIRTUAL_ENV/bin/python3` that never even ran `hooks/pre_bash_guard.py` could print `supertool-guard-v1 silent`, exit 0, and end the ladder walk right there, before a real interpreter further down it ever inspected the command. Nothing distinguished that outcome from an ordinary command the guard checked and had nothing to say about — `note` and `deny` both cost a forger *something visible*, and `silent` cost nothing at all. It is no longer a word `hooks/pre-bash-guard.sh`'s `attempt` will commit an answer to: a rung that prints it is treated exactly like one whose output does not match the wire protocol at all, discarded, and the walk continues to the next candidate — the same road [#1625](https://github.com/Digital-Process-Tools/claude-supertool/issues/1625) already cut for a forged `allow`.

**A bare `note` was the same gap wearing a real verb's name, and it stayed open through the whole of #1686** ([#2437](https://github.com/Digital-Process-Tools/claude-supertool/issues/2437)). `hooks/pre_bash_guard.py`'s own "nothing to say" used to cross as `note` with an empty body, and `$(...)` strips a trailing newline — so that genuine answer and a forger printing `supertool-guard-v1 note` with nothing after it, never touching `hooks/pre_bash_guard.py` at all, collapsed to the identical string. `attempt` now discards a bare `note` the same way it discards `silent`, and `hooks/pre_bash_guard.py`'s "nothing to say" moved to a dedicated third verb, `clean`, whose `additionalContext` `hooks/pre-bash-guard.sh` writes as a hardcoded empty string rather than reading from the rung at all — so no text a rung supplies, forged or genuine, ever reaches the transcript through it. This does not authenticate the channel; a forger who reads this file can print `clean` too. It closes the two guesses — `silent`, and the shortest legitimate word with nothing after it — that cost nothing to make without reading the patched source.
...
- **A found-but-malformed project `.supertool.json` fails the channel closed to `off`, distinguishably from a project that never configured Slack at all** ([#2427](https://github.com/Digital-Process-Tools/claude-supertool/issues/2427)). A truncated write or a leftover merge marker cannot be trusted to mean "no narrowing was declared" -- a project that had genuinely narrowed a channel and then broke its own config would otherwise silently widen back to the machine-owner's base level. `load_project_config_result()` returns the three states (found, absent, malformed) that `resolve_channel`'s `detail` renders apart.
...
| **Before the payload is read** | this decline — for a write-class builtin AND for any custom op this project's config declares under `"ops"` — runs the moment the op name is known, ahead of the `@file`/`@-` payload route ([#1878](https://github.com/Digital-Process-Tools/claude-supertool/issues/1878)). It used to sit after that route, so `edit:@-` or `git-commit:@-` under a mix still drained stdin before declining — `sys.stdin` is a single stream, so the caller had to re-send the whole payload to retry. The check needs only the op name, `argv` and the cwd, so nothing between "op is known" and "payload is loaded" was ever load-bearing for the decision. |
| **The remedy** | for a read-only op: run from the checkout you meant, or make `cwd:<that checkout>` the first op. For anything else (a write, or an "acts" op like `git-push`/`git-commit`/`gh-pr-merge`/`git-checkout`): run from *inside the checkout the write belongs to*, e.g. `python3 supertool.py git-push`. Do NOT use `cwd:<other checkout>` here — `cwd:PATH` really does `os.chdir(PATH)` for the rest of the call, so a write that follows it lands against the *other* checkout's own currently-checked-out branch, silently, with no further decline ([#2432](https://github.com/Digital-Process-Tools/claude-supertool/issues/2432)) |
